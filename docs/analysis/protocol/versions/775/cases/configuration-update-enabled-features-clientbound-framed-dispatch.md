...
clientbound `minecraft:update_enabled_features` framed dispatch/decode without
expanding it into feature registry hydration, enabled-feature semantics,
...
```text
client.jar ClientboundUpdateEnabledFeaturesPacket(Set<Identifier>)
          ...
          -> oracle/test-manifests/775/configuration_update_enabled_features_clientbound_framed_dispatch.test-manifest.json
            -> oracle/rust-tests/tests/oracle_contracts.rs
              -> packet::packet_by_id(775, Configuration, Clientbound, official id, official body)
                -> current result: UpdateEnabledFeatures compatibility identity and consumed body
```
...
| Field | Value |
|---|---|
| Case id | `configuration_update_enabled_features_clientbound_framed_dispatch` |
| Corridor | `Login -> Configuration -> Play` |
| Official source | `client.jar` `ClientboundUpdateEnabledFeaturesPacket(Set<Identifier>)`; `ClientboundUpdateEnabledFeaturesPacket.STREAM_CODEC`; `ConfigurationProtocols.CLIENTBOUND.codec().encode/decode(ClientboundUpdateEnabledFeaturesPacket)`; `ConfigurationProtocols.CLIENTBOUND_TEMPLATE.details().listPackets(...)`; `ClientboundUpdateEnabledFeaturesPacket.features()` |
...
| Generated answer | `oracle/answers/775/configuration_update_enabled_features_clientbound_framed_dispatch.answer.jsonl` |
| Test manifest | `oracle/test-manifests/775/configuration_update_enabled_features_clientbound_framed_dispatch.test-manifest.json` |
| Rust oracle test | `oracle/rust-tests/tests/oracle_contracts.rs::configuration_update_enabled_features_clientbound_framed_dispatch_matches_official_oracle_answer` |
...
| Failure packet | `oracle/failures/775/configuration_update_enabled_features_clientbound_framed_dispatch.why-what-answer.jsonl` |
| Rust fix task | `oracle/failures/775/configuration_update_enabled_features_clientbound_framed_dispatch.rust-fix-task.json` |
...
`minecraft:update_enabled_features` frame for one empty feature-set fixture,
dispatches that frame back to `ClientboundUpdateEnabledFeaturesPacket`,
preserves the empty feature set, and leaves no unread bytes after official
decode.
...
`minecraft:update_enabled_features` packet to the current compatibility alias
`Packet::PluginMessageClientbound` with channel `UpdateEnabledFeatures`,
preserving the packet identity while consuming the official feature collection
body bytes.
...
This does not prove feature registry hydration, enabled-feature semantics,
