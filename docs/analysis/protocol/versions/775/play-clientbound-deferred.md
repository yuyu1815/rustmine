...
| `0x64` | `minecraft:set_entity_link` | promoted BLUE | Official GameTest fixture creates a real source pig id `1` and destination armor stand id `2`; `ClientboundSetEntityLinkPacket(Entity, Entity)` stores those official entity ids. | Promoted only for this initialized fixture; reject other link ids. Do not fake linked entity existence or relationship semantics. |
...
| `0x6b` | `minecraft:set_passengers` | promoted BLUE | Official GameTest fixture creates a real minecart vehicle id `3` and passenger pig id `4` via `passenger.startRiding(vehicle)`; `ClientboundSetPassengersPacket(Entity)` writes vehicle id plus passenger id array. | Promoted only for vehicle `3` with exactly passenger `[4]`; reject other passenger topologies. Do not fake vehicle/passenger entity existence or relationship semantics. |
...
| `0x74` | `minecraft:sound_entity` | promoted BLUE | Official GameTest fixture creates source pig id `1` and uses `ClientboundSoundEntityPacket(SoundEvents.AMBIENT_CAVE, SoundSource.MASTER, Entity, 0.75F, 1.25F, 123456789L)`. | Promoted only for holder `8`, source `0`, entity id `1`, volume `0.75`, pitch `1.25`, seed `123456789`; reject other branches. Do not invent sound registry holder values or entity runtime context. |
...
| Entity relationship / metadata / movement / projectile | non-promoted branches of `0x64`, `0x6b`, `0x74`, and non-empty-relative branches of `0x7d` | Empty metadata `0x63`, empty attributes `0x83`, primitive projectile power `0x87`, primitive empty-relative teleport `0x7d`, bounded `0x84` mob-effect holder, and initialized GameTest fixtures for `0x64`, `0x6b`, and `0x74` have been promoted with bounded fixture policy. | Packet rows are promoted for the named fixtures; broader relationship/entity-sound/non-empty-relative branches still need new official fixtures. |
