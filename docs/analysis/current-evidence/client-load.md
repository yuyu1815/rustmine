...
| `play_entry` | `partial` | `play_bundle_delimiter_clientbound_framed_dispatch`; `play_add_entity_clientbound_framed_dispatch`; `play_animate_clientbound_framed_dispatch`; `play_award_stats_clientbound_framed_dispatch`; `play_block_changed_ack_clientbound_framed_dispatch` | Play clientbound bundle_delimiter framed dispatch/decode for the official registered empty-body singleton; Play clientbound add_entity framed dispatch/decode for one official built-in EntityType.PIG zero-movement fixture; Play clientbound animate framed dispatch/decode for one official STREAM_CODEC decode fixture with entity id 123 and SWING_MAIN_HAND; Play clientbound award_stats framed dispatch/decode for one official empty-stats fixture; Play clientbound block_changed_ack framed dispatch/decode for one official sequence fixture | successful entry into Play, bundle grouping behavior, arbitrary entity registry contents, initialized Entity/ServerEntity or Entity/Level behavior, entity existence, animation semantics, non-empty Stat registry entry decoding, stat semantics, UI/stat screen behavior, block prediction semantics, client world correction behavior, spawn readiness, world load, render readiness |
...
`play_block_changed_ack_clientbound_framed_dispatch`, jar-backed,
...
`minecraft:block_destruction` / `0x05`; first determine whether a smallest
official block_destruction fixture can be generated without initialized
