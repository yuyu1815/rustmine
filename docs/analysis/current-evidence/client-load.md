...
| `play_entry` | `partial` | `play_bundle_delimiter_clientbound_framed_dispatch`; `play_add_entity_clientbound_framed_dispatch`; `play_animate_clientbound_framed_dispatch`; `play_award_stats_clientbound_framed_dispatch`; `play_block_changed_ack_clientbound_framed_dispatch`; `play_block_destruction_clientbound_framed_dispatch`; `play_block_entity_data_clientbound_framed_dispatch`; `play_block_event_clientbound_framed_dispatch`; `play_block_update_clientbound_framed_dispatch`; `play_boss_event_clientbound_framed_dispatch`; `play_change_difficulty_clientbound_framed_dispatch`; `play_chunk_batch_finished_clientbound_framed_dispatch`; `play_chunk_batch_start_clientbound_framed_dispatch`; `play_chunks_biomes_clientbound_framed_dispatch`; `play_clear_titles_clientbound_framed_dispatch`; `play_command_suggestions_clientbound_framed_dispatch`; `play_commands_clientbound_framed_dispatch`; `play_container_close_clientbound_framed_dispatch`; `play_container_set_content_clientbound_framed_dispatch`; `play_container_set_data_clientbound_framed_dispatch`; `play_container_set_slot_clientbound_framed_dispatch`; `play_cookie_request_clientbound_framed_dispatch`; `play_cooldown_clientbound_framed_dispatch`; `play_custom_chat_completions_clientbound_framed_dispatch`; `play_custom_payload_clientbound_framed_dispatch`; `play_disconnect_clientbound_framed_dispatch`; `play_entity_position_sync_clientbound_framed_dispatch`; `play_forget_level_chunk_clientbound_framed_dispatch`; `play_game_event_clientbound_framed_dispatch`; `play_mount_screen_open_clientbound_framed_dispatch`; `play_hurt_animation_clientbound_framed_dispatch`; `play_initialize_border_clientbound_framed_dispatch`; `play_keep_alive_clientbound_framed_dispatch` | Play clientbound bundle_delimiter framed dispatch/decode for the official registered empty-body singleton; Play clientbound add_entity framed dispatch/decode for one official built-in EntityType.PIG zero-movement fixture; Play clientbound animate framed dispatch/decode for one official STREAM_CODEC decode fixture with entity id 123 and SWING_MAIN_HAND; Play clientbound award_stats framed dispatch/decode for one official empty-stats fixture; Play clientbound block_changed_ack framed dispatch/decode for one official sequence fixture; Play clientbound block_destruction framed dispatch/decode for one official breaker id, block position, and progress fixture; Play clientbound block_entity_data framed dispatch/decode for one official block position, built-in chest block entity type, and empty tag fixture; Play clientbound block_event framed dispatch/decode for one official block position, built-in note block, event type, and event data fixture; Play clientbound block_update framed dispatch/decode for one official block position and built-in stone default block-state fixture; Play clientbound boss_event framed dispatch/decode for one official UUID remove-operation fixture; Play clientbound change_difficulty framed dispatch/decode for one official difficulty/locked fixture; Play clientbound chunk_batch_finished framed dispatch/decode for one official batchSize fixture; Play clientbound chunk_batch_start framed dispatch/decode for the official singleton empty-body fixture; Play clientbound chunks_biomes framed dispatch/decode for the official empty chunkBiomeData-list fixture; Play clientbound clear_titles framed dispatch/decode for one official resetTimes=true boolean fixture; Play clientbound command_suggestions framed dispatch/decode for one official command id/range/empty-suggestions fixture; Play clientbound commands framed dispatch/decode for one official empty root-only command tree fixture; Play clientbound container_close framed dispatch/decode for one official container id fixture; Play clientbound container_set_content framed dispatch/decode for one official empty item list and empty carried stack fixture; Play clientbound container_set_data framed dispatch/decode for one official numeric fixture; Play clientbound container_set_slot framed dispatch/decode for one official empty ItemStack fixture; Play clientbound cookie_request framed dispatch/decode for one official Identifier key fixture; Play clientbound cooldown framed dispatch/decode for one official Identifier/duration fixture; Play clientbound custom_chat_completions framed dispatch/decode for one official ADD/string-list fixture; Play clientbound custom_payload framed dispatch/decode for one official BrandPayload fixture; Play clientbound disconnect framed dispatch/decode for one official empty literal Component fixture; Play clientbound entity_position_sync framed dispatch/decode for one official primitive position/movement fixture; Play clientbound forget_level_chunk framed dispatch/decode for one official chunk position fixture; Play clientbound game_event framed dispatch/decode for one official START_RAINING/0.5 fixture; Play clientbound mount_screen_open framed dispatch/decode for one official container/inventory/entity id fixture; Play clientbound hurt_animation framed dispatch/decode for one official entity id/yaw fixture; Play clientbound initialize_border framed dispatch/decode for one official primitive border-field fixture; Play clientbound keep_alive framed dispatch/decode for one official primitive id fixture | successful entry into Play, bundle grouping behavior, arbitrary entity registry contents, initialized Entity/ServerEntity or Entity/Level behavior, entity existence, animation semantics, non-empty Stat registry entry decoding, stat semantics, UI/stat screen behavior, block prediction semantics, client world correction behavior, block break animation semantics, entity existence for breaker id, client world state, block entity semantics, NBT schema, block event semantics, note block behavior, block update semantics, add/update boss-bar payloads, boss-bar UI behavior, difficulty UI behavior, hardcore semantics, chunk batch pacing behavior, chunk-load start semantics, chunk-load completion, non-empty chunk biome serialization, biome palette contents, initialized LevelChunk extraction, chunk/world state, title UI clearing behavior, title timing reset behavior, initialized client title/screen state, non-empty command suggestion Entry text/tooltip handling, Brigadier command tree semantics, command context behavior, command UI behavior, literal/argument command node semantics, custom command suggestions, non-empty ItemStack/component registry handling, menu property semantics, menu lifecycle behavior, screen close behavior, inventory state, cookie storage policy, cookie request/response runtime behavior, item cooldown semantics, item registry contents, chat UI behavior, arbitrary plugin-channel handling, payload routing policy, UI disconnect handling, entity position semantics, chunk unload behavior, game event semantics, initialized Level/player/weather state, mount entity existence, screen behavior, hurt animation semantics, world-border runtime behavior, runtime keep-alive response behavior, spawn readiness, world load, render readiness |
...
`play_mount_screen_open_clientbound_framed_dispatch`, jar-backed, regenerated in
