...
`play_remove_entities_clientbound_framed_dispatch` extend Play clientbound
packet-support proof across the next safe GREEN rows after `minecraft:ping` /
`0x3d`: `minecraft:pong_response` / `0x3e`,
`minecraft:player_abilities` / `0x40`,
`minecraft:player_combat_end` / `0x42`,
`minecraft:player_combat_enter` / `0x43`, and
`minecraft:remove_entities` / `0x4d`. These proofs are official framed
dispatch/decode fixtures only. They do not prove skipped recipe/chat/player-info
or movement rows, entity existence, combat runtime behavior, player ability
semantics, resource-pack behavior, render readiness, or client-load completion.
...
`play_set_border_warning_distance_clientbound_framed_dispatch` extend Play
clientbound packet-support proof across official border rows `0x58`-`0x5c`.
They prove primitive/context-free border center, lerp size, size, warning delay,
and warning distance body fixtures through official framed dispatch/decode and
Stevenarella full body consumption. They do not prove world-border runtime
behavior, warning UI behavior, world state, render readiness, or client-load
completion. `minecraft:set_entity_motion` / `0x65` remains intentionally
separate; the skipped YELLOW rows remain recorded in
`docs/analysis/protocol/versions/775/play-clientbound-deferred.md`.
