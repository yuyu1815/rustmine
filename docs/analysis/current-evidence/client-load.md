...
| `play_entry` | `partial` | `play_bundle_delimiter_clientbound_framed_dispatch`; `play_add_entity_clientbound_framed_dispatch`; `play_animate_clientbound_framed_dispatch` | Play clientbound bundle_delimiter framed dispatch/decode for the official registered empty-body singleton; Play clientbound add_entity framed dispatch/decode for one official built-in EntityType.PIG zero-movement fixture; Play clientbound animate framed dispatch/decode for one official STREAM_CODEC decode fixture with entity id 123 and SWING_MAIN_HAND | successful entry into Play, bundle grouping behavior, arbitrary entity registry contents, initialized Entity/ServerEntity or Entity/Level behavior, entity existence, animation semantics, spawn readiness, world load, render readiness |
...
`play_animate_clientbound_framed_dispatch`, jar-backed,
...
`minecraft:award_stats` / `0x03`; first determine whether a smallest official
award_stats fixture can be generated without initialized Minecraft/game state.
