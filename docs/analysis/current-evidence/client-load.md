...
| `play_entry` | `partial` | `play_bundle_delimiter_clientbound_framed_dispatch`; `play_add_entity_clientbound_framed_dispatch` | Play clientbound bundle_delimiter framed dispatch/decode for the official registered empty-body singleton; Play clientbound add_entity framed dispatch/decode for one official built-in EntityType.PIG zero-movement fixture | successful entry into Play, bundle grouping behavior, arbitrary entity registry contents, initialized Entity/ServerEntity behavior, spawn readiness, world load, render readiness |
...
order is `minecraft:animate` / `0x02`.
