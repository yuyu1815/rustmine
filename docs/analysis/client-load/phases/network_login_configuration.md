...
| Candidate evidence gap | Protocol 775 Handshaking serverbound packet-support table has its official `minecraft:intention` / `0x00` row; Configuration packet-support tables are complete through current official clientbound/serverbound rows; next missing packet-support surface is Login serverbound `minecraft:hello` / `0x00`; runtime Configuration-to-Play, registry hydration, and Play readiness remain later gaps. |
