...
| Candidate evidence gap | Protocol 775 Configuration clientbound packet-support table now has jar-backed rows through `minecraft:code_of_conduct` / `0x13`, the last current official clientbound table entry; runtime Configuration-to-Play, registry hydration, and Play readiness remain later gaps. |
