...
| Candidate evidence gap | Protocol 775 Handshaking serverbound packet-support table has its official `minecraft:intention` / `0x00` row; Login serverbound packet-support is complete through current official rows: `minecraft:hello` / `0x00`, `minecraft:key` / `0x01`, `minecraft:custom_query_answer` / `0x02`, `minecraft:login_acknowledged` / `0x03`, and `minecraft:cookie_response` / `0x04`; Login clientbound packet-support now has `minecraft:login_disconnect` / `0x00`, `minecraft:hello` / `0x01`, `minecraft:login_finished` / `0x02`, `minecraft:login_compression` / `0x03`, and `minecraft:custom_query` / `0x04`; the official table continues with `minecraft:cookie_request` / `0x05`; Configuration packet-support tables are complete through current official clientbound/serverbound rows; next missing packet-support surface is Login clientbound `minecraft:cookie_request` / `0x05` from `LoginProtocols.CLIENTBOUND_TEMPLATE`; runtime Configuration-to-Play, registry hydration, and Play readiness remain later gaps. |
...
## Current Login Key Packet Slice

```text
login_key_serverbound_framed_dispatch
  -> official Login serverbound minecraft:key frame
    from oracle/answers/775/login_key_serverbound_framed_dispatch.answer.jsonl
  -> packet::packet_by_id(775, State::Login, Direction::Serverbound, official id, body)
    -> current result: EncryptionResponse compatibility packet with full keybytes/encryptedChallenge body consumption
```

This slice is packet framing/dispatch/decode only. It stops before encryption
success, private-key validation, authentication success, login acknowledgement,
Configuration entry, or client-load completion.

## Current Login Custom Query Answer Packet Slice

```text
login_custom_query_answer_serverbound_framed_dispatch
  -> official Login serverbound minecraft:custom_query_answer frame
    from oracle/answers/775/login_custom_query_answer_serverbound_framed_dispatch.answer.jsonl
  -> packet::packet_by_id(775, State::Login, Direction::Serverbound, official id, body)
    -> current result: LoginPluginResponse compatibility packet with official transaction id and full null-payload marker body consumption
```

This slice is packet framing/dispatch/decode only. It stops before plugin
channel handling, custom payload semantics, Configuration entry, or
client-load completion.

## Current Login Acknowledged Packet Slice

```text
login_acknowledged_serverbound_framed_dispatch
  -> official Login serverbound minecraft:login_acknowledged frame
    from oracle/answers/775/login_acknowledged_serverbound_framed_dispatch.answer.jsonl
  -> packet::packet_by_id(775, State::Login, Direction::Serverbound, official id, body)
    -> current result: LoginAcknowledged compatibility packet with official empty-body singleton and full body consumption
```
...
## Current Login Cookie Response Packet Slice

```text
login_cookie_response_serverbound_framed_dispatch
  -> official Login serverbound minecraft:cookie_response frame
    from oracle/answers/775/login_cookie_response_serverbound_framed_dispatch.answer.jsonl
  -> packet::packet_by_id(775, State::Login, Direction::Serverbound, official id, body)
    -> current result: LoginCookieResponse packet with official Identifier key, non-null payload, and full body consumption
```

This slice is packet framing/dispatch/decode only. It stops before cookie
storage policy, cookie request/response runtime behavior, Configuration entry,
Play readiness, or client-load completion.

## Current Login Disconnect Packet Slice

```text
login_disconnect_clientbound_framed_dispatch
  -> official Login clientbound minecraft:login_disconnect frame
    from oracle/answers/775/login_disconnect_clientbound_framed_dispatch.answer.jsonl
  -> packet::packet_by_id(775, State::Login, Direction::Clientbound, official id, body)
    -> current result: LoginDisconnect compatibility packet with official empty literal Component reason and full body consumption
```

This slice is packet framing/dispatch/decode only. It stops before UI
disconnect handling, screen flow, authentication failure handling,
Configuration entry, Play readiness, or client-load completion.

## Current Login Clientbound Hello Packet Slice

```text
login_hello_clientbound_framed_dispatch
  -> official Login clientbound minecraft:hello frame
    from oracle/answers/775/login_hello_clientbound_framed_dispatch.answer.jsonl
  -> packet::packet_by_id(775, State::Login, Direction::Clientbound, official id, body)
    -> current result: EncryptionRequest_ShouldAuthenticate compatibility packet with official serverId, publicKey, challenge, shouldAuthenticate, and full body consumption
```
...
## Current Login Clientbound Finished Packet Slice

```text
login_finished_clientbound_framed_dispatch
  -> official Login clientbound minecraft:login_finished frame
    from oracle/answers/775/login_finished_clientbound_framed_dispatch.answer.jsonl
  -> packet::packet_by_id(775, State::Login, Direction::Clientbound, official id, body)
    -> current result: LoginSuccess_UUID compatibility packet with official GameProfile UUID/name and full empty-properties body consumption
```

This slice is packet framing/dispatch/decode only. It stops before
authentication success, Login-to-Configuration state transition handling,
profile property semantics, skin/session trust, Configuration entry,
Play readiness, or client-load completion.

## Current Login Clientbound Compression Packet Slice

```text
login_compression_clientbound_framed_dispatch
  -> official Login clientbound minecraft:login_compression frame
    from oracle/answers/775/login_compression_clientbound_framed_dispatch.answer.jsonl
  -> packet::packet_by_id(775, State::Login, Direction::Clientbound, official id, body)
    -> current result: SetInitialCompression compatibility packet with official VarInt compressionThreshold and full body consumption
```

This slice is packet framing/dispatch/decode only. It stops before compression
negotiation policy, connection compression activation, Login-to-Configuration
state transition handling, Configuration entry, Play readiness, or client-load
completion.

## Current Login Clientbound Custom Query Packet Slice

```text
login_custom_query_clientbound_framed_dispatch
  -> official Login clientbound minecraft:custom_query frame
    from oracle/answers/775/login_custom_query_clientbound_framed_dispatch.answer.jsonl
  -> packet::packet_by_id(775, State::Login, Direction::Clientbound, official id, body)
    -> current result: LoginPluginRequest compatibility packet with official transactionId, payload Identifier, empty payload body, and full body consumption
```

This slice is packet framing/dispatch/decode only. It stops before plugin
channel handling, custom query semantics, login acknowledgement behavior,
