...
| Field | Value |
|---|---|
| Current location | Protocol 775 `play_container_set_data_clientbound_framed_dispatch` packet-support package now passes: the exact oracle test validates the official Play clientbound `minecraft:container_set_data` / `0x13` answer for the official `ClientboundContainerSetDataPacket(int, int, int)` numeric fixture, body `070002012c`, framed bytes `13070002012c`, then decodes it through `packet::packet_by_id(775, State::Play, Direction::Clientbound, official id, body)` with full body consumption via the existing `Packet::WindowProperty` compatibility decoder. Play clientbound rows `minecraft:bundle_delimiter` / `0x00` through `minecraft:container_set_content` / `0x12` also pass. The generated 26.1.2 Play table audit observed 141 clientbound rows and 69 serverbound rows from `GameProtocols.CLIENTBOUND_TEMPLATE` / `SERVERBOUND_TEMPLATE`; first Play clientbound rows through the next target are `minecraft:bundle_delimiter` / `0x00`, `minecraft:add_entity` / `0x01`, `minecraft:animate` / `0x02`, `minecraft:award_stats` / `0x03`, `minecraft:block_changed_ack` / `0x04`, `minecraft:block_destruction` / `0x05`, `minecraft:block_entity_data` / `0x06`, `minecraft:block_event` / `0x07`, `minecraft:block_update` / `0x08`, `minecraft:boss_event` / `0x09`, `minecraft:change_difficulty` / `0x0a`, `minecraft:chunk_batch_finished` / `0x0b`, `minecraft:chunk_batch_start` / `0x0c`, `minecraft:chunks_biomes` / `0x0d`, `minecraft:clear_titles` / `0x0e`, `minecraft:command_suggestions` / `0x0f`, `minecraft:commands` / `0x10`, `minecraft:container_close` / `0x11`, `minecraft:container_set_content` / `0x12`, `minecraft:container_set_data` / `0x13`, and `minecraft:container_set_slot` / `0x14`. Configuration clientbound/serverbound, Handshaking serverbound, Login serverbound, and Login clientbound packet-support are complete through their current official rows. |
...
| Explicit uncertainty | `play_container_set_data_clientbound_framed_dispatch` proves only Play clientbound container_set_data packet id/body dispatch for one official numeric fixture. It does not prove menu property semantics, inventory state, initialized game state, runtime Configuration-to-Play transition, successful Play entry, world load, spawn readiness, render readiness, or client-load completion. |
      ...
      -> keep parent Codex responsible for user answers, route decisions,
        ...
        -> use subagents only for bounded evidence, mapping, review, or
          ...
          -> for packet-support loop, Play table audit now exists from
             ...
             minecraft:container_set_data / 0x13 proofs pass
            -> next packet-support target by the same official Play
               clientbound table order is minecraft:container_set_slot / 0x14
              -> first determine whether a smallest official container_set_slot fixture
