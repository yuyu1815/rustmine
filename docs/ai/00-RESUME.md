...
| Explicit uncertainty | `play_container_set_content_clientbound_framed_dispatch` proves only Play clientbound container_set_content packet id/body dispatch for one official empty item list and empty carried stack fixture. It does not prove non-empty ItemStack/component registry handling, menu lifecycle behavior, screen close behavior, inventory state, initialized game state, runtime Configuration-to-Play transition, successful Play entry, world load, spawn readiness, render readiness, or client-load completion. |
      ...
      -> keep parent Codex responsible for user answers, route decisions,
        ...
        -> use subagents only for bounded evidence, mapping, review, or
          ...
          -> for packet-support loop, Play table audit now exists from
             ...
             minecraft:container_set_content / 0x12 proofs pass
            -> next packet-support target by the same official Play
               clientbound table order is minecraft:container_set_data / 0x13
              -> first determine whether a smallest official container_set_data fixture
