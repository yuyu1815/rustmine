...
| Field | Value |
|---|---|
| Current location | Protocol 775 `login_key_serverbound_framed_dispatch` packet-support package now passes: the exact oracle test validates the official Login serverbound `minecraft:key` answer for one minimal keybytes/encryptedChallenge fixture and decodes it through `packet::packet_by_id(775, State::Login, Direction::Serverbound, official id, body)` with full body consumption. The generated 26.1.2 Configuration clientbound and serverbound packet tables are complete through their current official rows; the Handshaking serverbound table has its official `minecraft:intention` / `0x00` row; Login serverbound packet-support has `minecraft:hello` / `0x00` and `minecraft:key` / `0x01`. |
...
| Explicit uncertainty | `login_key_serverbound_framed_dispatch` proves only Login serverbound key packet id/body dispatch for one official minimal keybytes/encryptedChallenge fixture. It does not prove encryption success, private-key validation, authentication success, login acknowledgement, Configuration entry, runtime Configuration-to-Play transition, Play readiness, world load, render readiness, or client load completion. |
      ...
      -> keep parent Codex responsible for user answers, route decisions,
        ...
        -> use subagents only for bounded evidence, mapping, review, or
          ...
          -> for packet-support loop, network_login_configuration now has
             ...
             minecraft:hello / 0x00 and minecraft:key / 0x01
            -> next packet-support target is Login serverbound
               minecraft:custom_query_answer / 0x02, using LoginProtocols.SERVERBOUND_TEMPLATE
