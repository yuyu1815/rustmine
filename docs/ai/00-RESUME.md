...
| Field | Value |
|---|---|
| Current location | Protocol 775 Play clientbound packet-support now has safe GREEN/BLUE proofs through selected rows up to `minecraft:move_entity_rot` / `0x38`. The latest batch added jar-backed official answers and exact Rust oracle tests for `play_level_event_clientbound_framed_dispatch` (`minecraft:level_event` / `0x2e`, body `000007d10000007fffffe0400000000100`, framed `2e000007d10000007fffffe0400000000100`), `play_low_disk_space_warning_clientbound_framed_dispatch` (`minecraft:low_disk_space_warning` / `0x32`, empty body, framed `32`), `play_move_entity_pos_clientbound_framed_dispatch` (`minecraft:move_entity_pos` / `0x35`, body `7b1000f800008001`, framed `357b1000f800008001`), `play_move_entity_pos_rot_clientbound_framed_dispatch` (`minecraft:move_entity_pos_rot` / `0x36`, body `7c0010ffe0003040e000`, framed `367c0010ffe0003040e000`), and `play_move_entity_rot_clientbound_framed_dispatch` (`minecraft:move_entity_rot` / `0x38`, body `7d20f001`, framed `387d20f001`). Earlier Play clientbound packet-support passes include `minecraft:bundle_delimiter` / `0x00` through `minecraft:custom_payload` / `0x18`, plus safe rows `0x20`, `0x23`, `0x25`, `0x26`, `0x29`, and `0x2a`-`0x2c` where their case artifacts exist. A deferred ledger parks evidence-dependent YELLOW/RED rows including `0x19`-`0x1f`, `0x21`-`0x22`, `0x24`, `0x27`-`0x28`, and YELLOW rows `0x2d`, `0x2f`-`0x31`, `0x33`-`0x34`, `0x37`, `0x3b`, and `0x3c` until official fixture evidence exists. The generated 26.1.2 Play table audit observed 141 clientbound rows and 69 serverbound rows from `GameProtocols.CLIENTBOUND_TEMPLATE` / `SERVERBOUND_TEMPLATE`; the next safe GREEN/BLUE candidate by current cartography is `minecraft:move_vehicle` / `0x39`. Configuration clientbound/serverbound, Handshaking serverbound, Login serverbound, and Login clientbound packet-support are complete through their current official rows. |
        ...
        -> use subagents only for bounded evidence, mapping, review, or
          ...
          -> for packet-support loop, Play table audit now exists from
             ...
             minecraft:custom_chat_completions / 0x17 plus
	             ...
	             minecraft:move_entity_rot / 0x38 proofs pass; deferred ledger
