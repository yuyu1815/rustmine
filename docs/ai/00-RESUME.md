...
| Explicit uncertainty | `play_chunk_batch_start_clientbound_framed_dispatch` proves only Play clientbound chunk_batch_start packet id/body dispatch for the official singleton empty-body fixture. It does not prove chunk batch pacing behavior, chunk-load start semantics, chunk-load completion, initialized game state, runtime Configuration-to-Play transition, successful Play entry, world load, spawn readiness, render readiness, or client-load completion. |
      ...
      -> keep parent Codex responsible for user answers, route decisions,
        ...
        -> use subagents only for bounded evidence, mapping, review, or
          ...
          -> for packet-support loop, Play table audit now exists from
             ...
             minecraft:chunk_batch_start / 0x0c proofs pass
            -> next packet-support target by the same official Play
               clientbound table order is minecraft:chunks_biomes / 0x0d
              -> first determine whether a smallest official chunks_biomes fixture
