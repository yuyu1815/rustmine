...
| Explicit uncertainty | `play_command_suggestions_clientbound_framed_dispatch` proves only Play clientbound command_suggestions packet id/body dispatch for one official command id/range/empty-suggestions fixture. It does not prove non-empty suggestion Entry text/tooltip handling, Brigadier command tree semantics, command context behavior, command UI behavior, initialized game state, runtime Configuration-to-Play transition, successful Play entry, world load, spawn readiness, render readiness, or client-load completion. |
      ...
      -> keep parent Codex responsible for user answers, route decisions,
        ...
        -> use subagents only for bounded evidence, mapping, review, or
          ...
          -> for packet-support loop, Play table audit now exists from
             ...
             minecraft:command_suggestions / 0x0f proofs pass
            -> next packet-support target by the same official Play
               clientbound table order is minecraft:commands / 0x10
              -> first determine whether a smallest official commands fixture
