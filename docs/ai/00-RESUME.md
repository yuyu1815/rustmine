...
| Field | Value |
|---|---|
| Current location | Protocol 775 `login_finished_clientbound_framed_dispatch` packet-support package now passes: the exact oracle test validates the official Login clientbound `minecraft:login_finished` answer for one zero-UUID empty-name empty-properties GameProfile fixture and decodes it through `packet::packet_by_id(775, State::Login, Direction::Clientbound, official id, body)` with full body consumption. The generated 26.1.2 Configuration clientbound and serverbound packet tables are complete through their current official rows; the Handshaking serverbound table has its official `minecraft:intention` / `0x00` row; Login serverbound packet-support is complete through current official rows: `minecraft:hello` / `0x00`, `minecraft:key` / `0x01`, `minecraft:custom_query_answer` / `0x02`, `minecraft:login_acknowledged` / `0x03`, and `minecraft:cookie_response` / `0x04`; Login clientbound packet-support now has `minecraft:login_disconnect` / `0x00`, `minecraft:hello` / `0x01`, and `minecraft:login_finished` / `0x02`. |
...
| Explicit uncertainty | `login_finished_clientbound_framed_dispatch` proves only Login clientbound login_finished packet id/body dispatch for one official zero-UUID empty-name empty-properties GameProfile fixture. It does not prove authentication success, Login-to-Configuration state transition handling, profile property semantics, skin/session trust, Configuration entry, runtime Configuration-to-Play transition, Play readiness, world load, render readiness, or client load completion. |
      ...
      -> keep parent Codex responsible for user answers, route decisions,
        ...
        -> use subagents only for bounded evidence, mapping, review, or
          ...
          -> for packet-support loop, network_login_configuration now has
             ...
             and minecraft:login_finished / 0x02
            -> Login serverbound packet-support is complete through current
               ...
               minecraft:login_compression / 0x03, using
