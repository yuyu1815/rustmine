...
| Explicit uncertainty | `play_chunk_batch_finished_clientbound_framed_dispatch` proves only Play clientbound chunk_batch_finished packet id/body dispatch for one official batchSize fixture. It does not prove chunk batch pacing behavior, chunk-load completion, initialized game state, runtime Configuration-to-Play transition, successful Play entry, world load, spawn readiness, render readiness, or client-load completion. |
      ...
      -> keep parent Codex responsible for user answers, route decisions,
        ...
        -> use subagents only for bounded evidence, mapping, review, or
          ...
          -> for packet-support loop, Play table audit now exists from
             ...
             minecraft:chunk_batch_finished / 0x0b proofs pass
            -> next packet-support target by the same official Play
               clientbound table order is minecraft:chunk_batch_start / 0x0c
              -> first determine whether a smallest official chunk_batch_start fixture
