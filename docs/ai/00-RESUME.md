...
| Explicit uncertainty | `play_award_stats_clientbound_framed_dispatch` proves only Play clientbound award_stats packet id/body dispatch for one official empty-stats fixture. It does not prove non-empty Stat registry entry decoding, stat semantics, UI/stat screen behavior, runtime Configuration-to-Play transition, successful Play entry, world load, spawn readiness, render readiness, or client-load completion. |
      ...
      -> keep parent Codex responsible for user answers, route decisions,
        ...
        -> use subagents only for bounded evidence, mapping, review, or
          ...
          -> for packet-support loop, Play table audit now exists from
             ...
             plus minecraft:award_stats / 0x03 proofs pass
            -> next packet-support target by the same official Play
               clientbound table order is minecraft:block_changed_ack / 0x04
              -> first determine whether a smallest official block_changed_ack fixture
