...
| Current task | The parked-row follow-up batch promoted `0x62` clear `set_display_objective`, `0x6e` no-optional `set_score`, `0x71` empty-map `set_time`, and `0x86` empty-map `update_tags` into jar-backed oracle packages and Stevenarella dispatch mappings. The official Protocol 775 Play CLIENTBOUND table currently ends at `0x8c`. The immediate next unproven pointer remains `0x60`, and remaining parked rows stay parked unless a future official-source pass names safe fixtures. |
...
For the next Protocol 775 Play CLIENTBOUND task:
  ...
  -> especially parked: `0x60`-`0x61`, `0x63`-`0x64`, `0x66`, `0x6a`-`0x6d`, `0x70`, `0x72`, `0x74`-`0x75`, `0x79`-`0x7b`, `0x7d`-`0x7e`, `0x82`-`0x85`, `0x87`, `0x8a`, `0x8c`
