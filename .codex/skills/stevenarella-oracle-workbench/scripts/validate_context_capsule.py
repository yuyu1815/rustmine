#!/usr/bin/env python3
"""Validate a planner-to-leaf context capsule without third-party packages."""
...
READ_ONLY_ROLES = {
    "rustmine_nested_test_surface_mapper",
    "rustmine_nested_compatibility_critic",
}

RUST_ROLE = "rustmine_nested_rust_implementer"
DOCS_ROLE = "rustmine_nested_docs_rewriter"
DOCS_WRITE_PREFIXES = ("docs/analysis/", "docs/next/")
DOCS_WRITE_FILES = {"CONTEXT.md"}
...
GLOB_CHARS = set("*?[]{}")
SCHEMA_PATH = Path(__file__).resolve().parents[1] / "schemas" / "context-capsule.schema.json"


def fail(message: str) -> None:
    raise SystemExit(f"context capsule invalid: {message}")
...
def load_contract_constants() -> dict[str, object]:
    ...
    if not isinstance(schema, dict):
        fail("context capsule schema is not an object")
    try:
        role_enum = schema["properties"]["agent_role"]["enum"]
        batch_props = schema["properties"]["batch"]["properties"]
        baseline_props = schema["properties"]["write_policy"]["properties"]["diff_baseline"]["properties"]
        required_evidence_items = schema["properties"]["required_evidence"]["items"]
        return_contract_props = schema["properties"]["return_contract"]["properties"]
    except KeyError as e:
        fail(f"context capsule schema is missing expected key: {e}")
    return {
        "roles": set(role_enum),
        "max_leaf_count": batch_props["leaf_count"]["maximum"],
        "baseline": {
        ...
        "return_contract_props": return_contract_props,
    }
...
def check_path_entries(entries: object, *, write: bool) -> set[str]:
    if not isinstance(entries, list):
        fail("allowed_reads/allowed_writes must be arrays")
    seen: set[str] = set()
...
def validate_capsule(capsule: object, constants: dict[str, object]) -> tuple[str, set[str]]:
    ...
    if capsule["schema_version"] != "context-capsule/v1":
        fail("schema_version must be context-capsule/v1")
    role = capsule["agent_role"]
    roles = constants["roles"]
    if role not in roles:
        fail(f"unknown agent_role: {role}")

    batch = capsule["batch"]
    if not isinstance(batch, dict):
        fail("batch must be an object")
    if set(batch) != {"id", "leaf_index", "leaf_count"}:
        fail("batch must contain only id, leaf_index, and leaf_count")
    if not isinstance(batch["id"], str) or not batch["id"]:
        fail("batch.id must be a non-empty string")
    leaf_index = batch["leaf_index"]
    leaf_count = batch["leaf_count"]
    if not isinstance(leaf_index, int) or not isinstance(leaf_count, int):
        fail("batch leaf_index and leaf_count must be integers")
    max_leaf_count = constants["max_leaf_count"]
    if leaf_count < 1 or leaf_count > max_leaf_count:
        fail(f"batch.leaf_count must be between 1 and {max_leaf_count}")
    if leaf_index < 1 or leaf_index > leaf_count:
        fail("batch.leaf_index must be between 1 and leaf_count")
    ...
    expected_policy = {"mode", "post_run_diff_check", "diff_baseline"}
    ...
    baseline = policy["diff_baseline"]
    if not isinstance(baseline, dict):
        fail("write_policy.diff_baseline must be an object")
    expected_baseline = constants["baseline"]
    ...
    if role == DOCS_ROLE:
        if policy["mode"] != "workspace_write" or not writes:
            fail(f"{DOCS_ROLE} must be workspace_write with explicit allowed_writes")
        if "fragile_preconditions" not in capsule:
            fail(f"{DOCS_ROLE} capsules must include supplied wording or exact rewrite intent in fragile_preconditions")
        invalid = sorted(
            path
            for path in writes
            if path not in DOCS_WRITE_FILES
            and not any(path.startswith(prefix) for prefix in DOCS_WRITE_PREFIXES)
        )
        if invalid:
            fail(
                f"{DOCS_ROLE} writes are limited to docs/analysis/, docs/next/, or CONTEXT.md: {invalid}"
            )
    ...
    if "fragile_preconditions" in capsule:
        validate_string_array(capsule["fragile_preconditions"], name="fragile_preconditions")
    ...
    for key in ["must_report", "must_not_claim"]:
        validate_string_array(contract[key], name=f"return_contract.{key}")
...
def validate_rust_subset(rust_task: object, capsule_writes: set[str]) -> None:
    ...
    if extra:
        fail(f"rust allowed_write_scope is outside capsule allowed_writes: {extra}")


def main(argv: list[str]) -> int:
    if len(argv) not in {2, 3}:
        print(
            "usage: validate_context_capsule.py CAPSULE_JSON [RUST_FIX_TASK_JSON]\n"
            "RUST_FIX_TASK_JSON is required when agent_role is rustmine_nested_rust_implementer.",
            file=sys.stderr,
        )
        return 2
    ...
    if len(argv) == 3:
        if role != RUST_ROLE:
            fail("RUST_FIX_TASK_JSON may only be supplied for the Rust nested role")
        validate_rust_subset(load_json(argv[2]), capsule_writes)
    print("context capsule valid")
    return 0
