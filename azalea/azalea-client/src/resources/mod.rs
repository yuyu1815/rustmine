...
const INITIAL_MOJANG_LOGO_ID: &str = "minecraft:textures/gui/title/mojangstudios.png";
const INITIAL_MOJANG_LOGO_RESOURCE: &str = "assets/minecraft/textures/gui/title/mojangstudios.png";
const INITIAL_CUBEMAP_ID: &str = "minecraft:textures/gui/title/background/panorama";
const INITIAL_CUBEMAP_RESOURCE_PREFIX: &str =
    "assets/minecraft/textures/gui/title/background/panorama";
const INITIAL_CUBEMAP_SUFFIXES: &[&str] =
    &["_1.png", "_3.png", "_5.png", "_4.png", "_0.png", "_2.png"];
...
#[derive(Clone, Debug, PartialEq)]
pub struct ClientEquipmentRenderAssetCandidateSet {
    equipment: Vec<ClientEquipmentRenderAssetCandidate>,
}

impl ClientEquipmentRenderAssetCandidateSet {
    pub fn equipment(&self) -> &[ClientEquipmentRenderAssetCandidate] {
        &self.equipment
    }

    pub fn reports(&self) -> impl Iterator<Item = ClientEquipmentRenderAssetCandidateReport> + '_ {
        self.equipment
            .iter()
            .map(ClientEquipmentRenderAssetCandidate::report)
    }
    ...
    pub fn layer_count(&self) -> usize {
        self.equipment
            .iter()
            .map(ClientEquipmentRenderAssetCandidate::layer_count)
            .sum()
    }
    ...
    pub fn texture_count(&self) -> usize {
        self.equipment
            .iter()
            .map(ClientEquipmentRenderAssetCandidate::texture_count)
            .sum()
    }
    ...
    pub fn available_resource_count(&self) -> usize {
        self.equipment
            .iter()
            .map(ClientEquipmentRenderAssetCandidate::available_resource_count)
            .sum()
    }
    ...
    pub fn decoded_resource_count(&self) -> usize {
        self.equipment
            .iter()
            .map(ClientEquipmentRenderAssetCandidate::decoded_resource_count)
            .sum()
    }
    ...
    pub fn blocker_count(&self) -> usize {
        self.equipment
            .iter()
            .map(ClientEquipmentRenderAssetCandidate::blocker_count)
            .sum()
    }
    ...
    pub fn byte_count(&self) -> usize {
        self.equipment
            .iter()
            .map(ClientEquipmentRenderAssetCandidate::byte_count)
            .sum()
    }

    pub fn summary_fragment(&self) -> String {
        format!(
            "equipment_render_asset_candidates:equipment:{} layers:{} textures:{} available:{} decoded:{} blocked:{} bytes:{} boundary:{}",
    ...
    pub fn items(&self) -> Vec<String> {
        ...
        items.extend(
            self.reports()
                .map(|report| equipment_render_asset_candidate_report_item(&report)),
        );
        items
    }
}
...
#[derive(Clone, Debug, PartialEq)]
pub struct ClientEquipmentRenderAssetCandidate {
    equipment_id: String,
    resource: String,
    pack_id: String,
    layers: Vec<ClientEquipmentRenderLayerCandidate>,
}

impl ClientEquipmentRenderAssetCandidate {
    ...
    pub fn layers(&self) -> &[ClientEquipmentRenderLayerCandidate] {
        &self.layers
    }
    ...
    pub fn texture_count(&self) -> usize {
        self.layers
            .iter()
            .map(ClientEquipmentRenderLayerCandidate::texture_count)
            .sum()
    }
    ...
    pub fn available_resource_count(&self) -> usize {
        self.layers
            .iter()
            .map(ClientEquipmentRenderLayerCandidate::available_resource_count)
            .sum()
    }
    ...
    pub fn decoded_resource_count(&self) -> usize {
        self.layers
            .iter()
            .map(ClientEquipmentRenderLayerCandidate::decoded_resource_count)
            .sum()
    }
    ...
    pub fn blocker_count(&self) -> usize {
        self.layers
            .iter()
            .map(ClientEquipmentRenderLayerCandidate::blocker_count)
            .sum()
    }
    ...
    pub fn byte_count(&self) -> usize {
        self.layers
            .iter()
            .map(ClientEquipmentRenderLayerCandidate::byte_count)
            .sum()
    }
    ...
    pub fn report(&self) -> ClientEquipmentRenderAssetCandidateReport {
        ClientEquipmentRenderAssetCandidateReport {
...
#[derive(Clone, Debug, PartialEq)]
pub struct ClientEquipmentRenderLayerCandidate {
    layer_type: String,
    textures: Vec<ClientEquipmentRenderTextureCandidate>,
    decoded_texture_resources: Vec<ClientEquipmentRenderDecodedTextureResource>,
    blockers: Vec<ClientEquipmentRenderAssetBlocker>,
}

impl ClientEquipmentRenderLayerCandidate {
    pub fn layer_type(&self) -> &str {
        &self.layer_type
    }

    pub fn textures(&self) -> &[ClientEquipmentRenderTextureCandidate] {
        &self.textures
    }

    pub fn decoded_texture_resources(&self) -> &[ClientEquipmentRenderDecodedTextureResource] {
        &self.decoded_texture_resources
    }

    pub fn blockers(&self) -> &[ClientEquipmentRenderAssetBlocker] {
        &self.blockers
    }
    ...
    pub fn available_resource_count(&self) -> usize {
        self.decoded_texture_resources.len()
            + self
                .blockers
                .iter()
                .filter(|blocker| blocker.selected_pack_id.is_some())
                .count()
    }
    ...
    pub fn byte_count(&self) -> usize {
        self.decoded_texture_resources
            .iter()
            .map(ClientEquipmentRenderDecodedTextureResource::byte_count)
            .sum()
    }
}

#[derive(Clone, Debug, PartialEq)]
pub struct ClientEquipmentRenderTextureCandidate {
    texture_id: String,
...
impl ClientEquipmentRenderTextureCandidate {
    ...
    fn item_fragment(&self) -> String {
        format!(
            "{}={} resource:{} dyeable:{} player_texture:{}",
            self.texture_id,
            self.texture_resource_id,
            self.resource,
            equipment_dyeable_fragment(self.dyeable.as_ref()),
            self.use_player_texture.unwrap_or(false)
        )
    }
}

#[derive(Clone, Debug, PartialEq)]
pub struct ClientEquipmentRenderDecodedTextureResource {
    layer_type: String,
    texture_id: String,
    texture_resource_id: String,
    resource: String,
...
impl ClientEquipmentRenderDecodedTextureResource {
    pub fn layer_type(&self) -> &str {
        &self.layer_type
    }
    ...
    fn item_fragment(&self) -> String {
        format!(
            "{}:{}={}@{}:{} bytes:png:{}x{} dyeable:{} player_texture:{}",
            self.layer_type,
            self.texture_id,
            self.resource,
            self.pack_id,
...
#[derive(Clone, Debug, PartialEq)]
pub struct ClientEquipmentRenderAssetBlocker {
    layer_type: String,
    texture_id: String,
    texture_resource_id: String,
    resource: String,
    ...
    dyeable: Option<ClientEquipmentDyeable>,
    use_player_texture: Option<bool>,
    reason: ClientEquipmentRenderAssetBlockerReason,
}

impl ClientEquipmentRenderAssetBlocker {
    pub fn layer_type(&self) -> &str {
        &self.layer_type
    }
    ...
    pub fn reason(&self) -> &ClientEquipmentRenderAssetBlockerReason {
        &self.reason
    }
...
#[derive(Clone, Debug, PartialEq)]
pub enum ClientEquipmentRenderAssetBlockerReason {
...
impl ClientEquipmentRenderAssetBlockerReason {
    fn report_fragment(&self) -> String {
        match self {
...
#[derive(Clone, Debug, PartialEq)]
pub struct ClientEquipmentRenderAssetCandidateReport {
    equipment_id: String,
    resource: String,
    pack_id: String,
    layer_count: usize,
...
impl ClientEquipmentRenderAssetCandidateReport {
    pub fn equipment_id(&self) -> &str {
        &self.equipment_id
    }
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ClientWaypointStyleAssetSet {
    styles: Vec<ClientWaypointStyleAssetCandidate>,
}

impl ClientWaypointStyleAssetSet {
    pub fn styles(&self) -> &[ClientWaypointStyleAssetCandidate] {
        &self.styles
    }

    pub fn reports(&self) -> impl Iterator<Item = ClientWaypointStyleAssetReport> + '_ {
        self.styles
            .iter()
            .map(ClientWaypointStyleAssetCandidate::report)
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ClientWaypointStyleAssetCandidate {
    style_id: String,
    resource: String,
    pack_id: String,
    sprites: Vec<String>,
    sprite_locations: Vec<String>,
    available_resources: Vec<ClientWaypointStyleAssetAvailability>,
    blockers: Vec<ClientWaypointStyleAssetBlocker>,
}

impl ClientWaypointStyleAssetCandidate {
    ...
    pub fn available_resources(&self) -> &[ClientWaypointStyleAssetAvailability] {
        &self.available_resources
    }

    pub fn blockers(&self) -> &[ClientWaypointStyleAssetBlocker] {
        &self.blockers
    }
    ...
    pub fn report(&self) -> ClientWaypointStyleAssetReport {
        ClientWaypointStyleAssetReport {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ClientWaypointStyleAssetAvailability {
    sprite_location: String,
    resource: String,
    pack_id: String,
}

impl ClientWaypointStyleAssetAvailability {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ClientWaypointStyleAssetBlocker {
    sprite_location: String,
    resource: String,
    reason: ClientWaypointStyleAssetBlockerReason,
}

impl ClientWaypointStyleAssetBlocker {
    pub fn sprite_location(&self) -> &str {
        &self.sprite_location
    }
    ...
    pub fn reason(&self) -> ClientWaypointStyleAssetBlockerReason {
        self.reason
    }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ClientWaypointStyleAssetBlockerReason {
    MissingTextureResource,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ClientWaypointStyleAssetReport {
    style_id: String,
    resource: String,
    pack_id: String,
    sprite_count: usize,
...
impl ClientWaypointStyleAssetReport {
    pub fn style_id(&self) -> &str {
        &self.style_id
    }
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ClientSoundPreloadBufferCandidate {
    event_id: String,
    sound_name: String,
    file_resource: String,
    pack_id: Option<String>,
    byte_count: Option<usize>,
    missing_blockers: Vec<String>,
}

impl ClientSoundPreloadBufferCandidate {
    fn available(
        event_id: impl Into<String>,
        sound_name: impl Into<String>,
        file_resource: impl Into<String>,
        pack_id: impl Into<String>,
        byte_count: usize,
    ) -> Self {
        Self {
            ...
            file_resource: file_resource.into(),
    ...
    fn blocked(
        ...
        sound_name: impl Into<String>,
        file_resource: impl Into<String>,
        missing_blocker: impl Into<String>,
    ) -> Self {
        Self {
            event_id: event_id.into(),
            sound_name: sound_name.into(),
            ...
            pack_id: None,
            byte_count: None,
            missing_blockers: vec![missing_blocker.into()],
        }
    }
    ...
    pub fn missing_blockers(&self) -> &[String] {
        &self.missing_blockers
    }
}
...
pub struct ClientSoundEventsReloadReport {
    ...
    preload_buffer_candidates: Vec<ClientSoundPreloadBufferCandidate>,
...
impl ClientSoundEventsReloadReport {
    ...
    pub fn preload_buffer_candidates(&self) -> &[ClientSoundPreloadBufferCandidate] {
        &self.preload_buffer_candidates
    }
    ...
    pub fn summary_fragment(&self) -> String {
        ...
        let preload_file_resources = self
            ...
            .collect::<Vec<_>>()
            .join(",");
        let preload_buffer_blocker_count = self
            .preload_buffer_candidates
            .iter()
            .map(|candidate| candidate.missing_blockers.len())
            .sum::<usize>();
        format!(
            "namespaces:{} sounds_json:{} events:{} entries:{} files:{} event_refs:{} missing_files:{} preloads:{} preload_candidates:{} preload_blockers:{} resources:{} file_resources:{} preload_resources:{} missing_resources:{}",
            self.namespace_count,
            self.sounds_json_resource_count,
            self.event_count,
            self.sound_entry_count,
            ...
            self.missing_file_sound_count,
            self.preload_file_sound_count,
            self.preload_buffer_candidates.len(),
            preload_buffer_blocker_count,
...
fn load_client_sound_events_from_resources(
    ...
    let mut preload_buffer_candidates = Vec::<ClientSoundPreloadBufferCandidate>::new();
        ...
        for location in stack.resource_stack(&resource) {
            ...
            for (event_key, value) in sound_events {
                ...
                let event = events
                    ...
                    .or_insert_with(|| ClientSoundEvent {
                            ...
                            if let Some(location) = sound_file_cache.get(file_resource) {
                                ...
                                if entry.preload {
                                    ...
                                    preload_file_resources
                                        .entry(file_resource.clone())
                                        .or_insert(file);
                                    preload_buffer_candidates.push(preload_buffer_candidate(
                                        &event_id,
                                        &entry,
                                        file_resource,
                                        location,
                                ...
                                if entry.preload {
                                    preload_buffer_candidates.push(
                                        missing_preload_buffer_candidate(
                                            &event_id,
                                            &entry,
                                            file_resource,
                                        ),
...
fn preload_buffer_candidate(
    event_id: &str,
    entry: &ClientSoundEntry,
    file_resource: &str,
    location: &ResourceLocation,
) -> ResourceReloadResult<ClientSoundPreloadBufferCandidate> {
    let byte_count = location
        .read_bytes()
        .map_err(|source| ResourceReloadError::ReadResource {
            resource: file_resource.to_owned(),
            path: location.path.clone(),
            source,
        })?
        .len();

    Ok(ClientSoundPreloadBufferCandidate::available(
        event_id,
        &entry.name,
        file_resource,
        &location.pack_id,
...
fn missing_preload_buffer_candidate(
    event_id: &str,
    entry: &ClientSoundEntry,
    file_resource: &str,
) -> ClientSoundPreloadBufferCandidate {
    ClientSoundPreloadBufferCandidate::blocked(
        event_id,
        &entry.name,
        file_resource,
        format!("missing top-priority resource `{file_resource}`"),
    )
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WaypointStyleAssetReloadListener {
    ids: Vec<String>,
}

impl WaypointStyleAssetReloadListener {
    pub fn new(ids: impl IntoIterator<Item = impl Into<String>>) -> Self {
        Self {
            ids: ids.into_iter().map(Into::into).collect(),
        }
    ...
    pub fn load(
        &self,
        stack: &ClientResourceStack,
    ) -> ResourceReloadResult<ClientWaypointStyleAssetSet> {
        load_client_waypoint_style_assets(stack, &self.ids)
    }
}

impl Default for WaypointStyleAssetReloadListener {
    fn default() -> Self {
        Self::new(DEFAULT_WAYPOINT_STYLE_MANIFEST_IDS.iter().copied())
    }
}

impl ResourceReloadListener for WaypointStyleAssetReloadListener {
    fn name(&self) -> &str {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EquipmentRenderAssetCandidateReloadListener {
    directory: String,
}

impl EquipmentRenderAssetCandidateReloadListener {
    pub fn new(directory: impl Into<String>) -> Self {
        Self {
            directory: directory.into(),
        }
...
pub fn load_client_waypoint_style_assets(
    stack: &ClientResourceStack,
    ids: &[String],
) -> ResourceReloadResult<ClientWaypointStyleAssetSet> {
    let styles = load_client_waypoint_styles(stack, ids)?;
...
pub fn load_client_equipment_render_asset_candidates(
    stack: &ClientResourceStack,
    directory: &str,
) -> ResourceReloadResult<ClientEquipmentRenderAssetCandidateSet> {
    let assets = load_client_equipment_assets(stack, directory)?;
    Ok(build_client_equipment_render_asset_candidate_set(
        stack, &assets,
    ))
}
...
) -> ClientEquipmentRenderAssetCandidateSet {
    let equipment = assets
        .assets()
        .iter()
        .map(|asset| build_client_equipment_render_asset_candidate(stack, asset))
        .collect();

    ClientEquipmentRenderAssetCandidateSet { equipment }
}

fn build_client_equipment_render_asset_candidate(
    stack: &ClientResourceStack,
    asset: &ClientEquipmentAsset,
) -> ClientEquipmentRenderAssetCandidate {
    let report = asset.report();
    let layers = asset
        .layers()
        .iter()
        .map(|layer| build_client_equipment_render_layer_candidate(stack, layer))
        .collect();

    ClientEquipmentRenderAssetCandidate {
        equipment_id: asset.id().to_owned(),
        resource: report.resource().to_owned(),
        pack_id: report.pack_id().to_owned(),
        layers,
    }
...
fn build_client_equipment_render_layer_candidate(
    stack: &ClientResourceStack,
    layer: &ClientEquipmentLayer,
) -> ClientEquipmentRenderLayerCandidate {
    let mut textures = Vec::new();
    let mut decoded_texture_resources = Vec::new();
    let mut blockers = Vec::new();

    for entry in layer.entries() {
        let texture_resource_id =
            equipment_layer_texture_location(layer.layer_type(), entry.texture());
        let resource = resource_path_for_identifier(&texture_resource_id);
        let texture = ClientEquipmentRenderTextureCandidate {
            texture_id: entry.texture().to_owned(),
            texture_resource_id: texture_resource_id.clone(),
            resource: resource.clone(),
            dyeable: entry.dyeable().cloned(),
            use_player_texture: entry.use_player_texture(),
        };

        match stack.find_resource(&resource) {
            Some(location) => match load_equipment_render_decoded_texture_resource(
                layer.layer_type(),
                entry,
                &texture_resource_id,
                &resource,
                ...
                Ok(decoded) => decoded_texture_resources.push(decoded),
                Err(blocker) => blockers.push(blocker),
            },
            None => blockers.push(ClientEquipmentRenderAssetBlocker {
                layer_type: layer.layer_type().to_owned(),
                texture_id: entry.texture().to_owned(),
                texture_resource_id: texture_resource_id.clone(),
                resource: resource.clone(),
                ...
                dyeable: entry.dyeable().cloned(),
                use_player_texture: entry.use_player_texture(),
                reason: ClientEquipmentRenderAssetBlockerReason::MissingTextureResource,
            }),
        }
...
fn load_equipment_render_decoded_texture_resource(
    layer_type: &str,
    entry: &ClientEquipmentLayerEntry,
    texture_resource_id: &str,
    resource: &str,
    location: ResourceLocation,
) -> Result<ClientEquipmentRenderDecodedTextureResource, ClientEquipmentRenderAssetBlocker> {
    let pack_id = location.pack_id.clone();
    let bytes = match read_resource_bytes(resource, &location) {
        Ok(bytes) => bytes,
        Err(_) => {
            return Err(ClientEquipmentRenderAssetBlocker {
                layer_type: layer_type.to_owned(),
                texture_id: entry.texture().to_owned(),
                texture_resource_id: texture_resource_id.to_owned(),
                resource: resource.to_owned(),
                selected_pack_id: Some(pack_id),
                byte_count: None,
                dyeable: entry.dyeable().cloned(),
                use_player_texture: entry.use_player_texture(),
                reason: ClientEquipmentRenderAssetBlockerReason::ReadTextureResource,
            });
        }
    };
    ...
    let image = match decode_png_dimensions(resource, &location, &bytes) {
        ...
        Err(ResourceReloadError::InvalidPngSignature { .. }) => {
            return Err(ClientEquipmentRenderAssetBlocker {
                ...
                dyeable: entry.dyeable().cloned(),
                use_player_texture: entry.use_player_texture(),
                reason: ClientEquipmentRenderAssetBlockerReason::InvalidPngSignature,
            });
        }
        Err(ResourceReloadError::InvalidPngMetadata { reason, .. }) => {
            return Err(ClientEquipmentRenderAssetBlocker {
                ...
                dyeable: entry.dyeable().cloned(),
                use_player_texture: entry.use_player_texture(),
                reason: ClientEquipmentRenderAssetBlockerReason::InvalidPngMetadata { reason },
            });
        }
        Err(_) => {
            return Err(ClientEquipmentRenderAssetBlocker {
    ...
    Ok(ClientEquipmentRenderDecodedTextureResource {
        layer_type: layer_type.to_owned(),
        texture_id: entry.texture().to_owned(),
        texture_resource_id: texture_resource_id.to_owned(),
        resource: resource.to_owned(),
...
fn build_client_waypoint_style_asset_set(
    stack: &ClientResourceStack,
    styles: &ClientWaypointStyleSet,
) -> ClientWaypointStyleAssetSet {
    ClientWaypointStyleAssetSet {
        styles: styles
            .styles()
            .iter()
            .map(|style| build_client_waypoint_style_asset_candidate(stack, style))
            .collect(),
    }
}

fn build_client_waypoint_style_asset_candidate(
    stack: &ClientResourceStack,
    style: &ClientWaypointStyle,
) -> ClientWaypointStyleAssetCandidate {
    let mut available_resources = Vec::new();
    let mut blockers = Vec::new();

    for sprite_location in style.sprite_locations() {
        let resource = waypoint_style_sprite_resource_path_for_location(sprite_location);
        if let Some(location) = stack.find_resource(&resource) {
            available_resources.push(ClientWaypointStyleAssetAvailability {
                sprite_location: sprite_location.clone(),
                resource,
                pack_id: location.pack_id,
            });
        } else {
            blockers.push(ClientWaypointStyleAssetBlocker {
                sprite_location: sprite_location.clone(),
                resource,
                reason: ClientWaypointStyleAssetBlockerReason::MissingTextureResource,
            });
        }
    }

    ClientWaypointStyleAssetCandidate {
        style_id: style.id().to_owned(),
        resource: style.report().resource().to_owned(),
        pack_id: style.report().pack_id().to_owned(),
...
fn equipment_render_asset_candidate_report_item(
    report: &ClientEquipmentRenderAssetCandidateReport,
) -> String {
    format!(
        "{} id:{} layers:{} textures:{} available:{} decoded:{} bytes:{} blockers:{} layer_candidates:{} resources:{} blocker_details:{} boundary:{}",
        ...
        equipment_render_layer_candidates_fragment(report.layers()),
...
fn equipment_render_layer_candidates_fragment(
    layers: &[ClientEquipmentRenderLayerCandidate],
) -> String {
    if layers.is_empty() {
        ...
        .map(|layer| {
            format!(
                ...
                layer
                    .textures()
                    .iter()
                    .map(ClientEquipmentRenderTextureCandidate::item_fragment)
                    .collect::<Vec<_>>()
                    .join("|")
            )
        })
...
fn equipment_render_resources_fragment(layers: &[ClientEquipmentRenderLayerCandidate]) -> String {
    let resources = layers
        .iter()
        .flat_map(ClientEquipmentRenderLayerCandidate::decoded_texture_resources)
        .collect::<Vec<_>>();
    if resources.is_empty() {
        return "none".to_owned();
    }
    ...
    resources
        .iter()
        .map(|resource| resource.item_fragment())
        .collect::<Vec<_>>()
        .join("|")
}

fn equipment_render_blockers_fragment(layers: &[ClientEquipmentRenderLayerCandidate]) -> String {
    let blockers = layers
        .iter()
        .flat_map(ClientEquipmentRenderLayerCandidate::blockers)
        .collect::<Vec<_>>();
...
fn equipment_dyeable_fragment(dyeable: Option<&ClientEquipmentDyeable>) -> String {
    match dyeable {
        Some(dyeable) => dyeable
            .color_when_undyed()
            .map(|color| format!("color_when_undyed:{color}"))
            .unwrap_or_else(|| "color_when_undyed:none".to_owned()),
        None => "none".to_owned(),
    }
}
...
fn waypoint_style_asset_report_item(report: &ClientWaypointStyleAssetReport) -> String {
    format!(
        "{} id:{} sprites:{}:{} locations:{} available:{} missing:{} resources:{} blockers:{}",
        report.loaded_resource_pack(),
        report.style_id(),
        report.sprite_count(),
        particle_sprite_ids_fragment(report.sprites()),
        ...
        report.available_resource_count(),
        report.missing_resource_count(),
        waypoint_style_asset_availability_fragment(report.available_resources()),
        waypoint_style_asset_blockers_fragment(report.blockers())
    )
}

fn waypoint_style_sprite_locations_fragment(sprite_locations: &[String]) -> String {
    if sprite_locations.is_empty() {
        return "none".to_owned();
    }
    sprite_locations.join(",")
}

fn waypoint_style_asset_availability_fragment(
    available_resources: &[ClientWaypointStyleAssetAvailability],
) -> String {
    if available_resources.is_empty() {
        return "none".to_owned();
    }
...
fn waypoint_style_asset_blockers_fragment(blockers: &[ClientWaypointStyleAssetBlocker]) -> String {
    if blockers.is_empty() {
        return "none".to_owned();
    }
    blockers
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CloudRendererRebuildCandidateReloadListener {
    resource: String,
}

impl CloudRendererRebuildCandidateReloadListener {
    pub fn new(resource: impl Into<String>) -> Self {
        Self {
            resource: resource.into(),
        }
    ...
    pub fn load(&self, stack: &ClientResourceStack) -> ClientCloudRendererRebuildCandidateReport {
        load_cloud_renderer_rebuild_candidate_report(stack, &self.resource)
    }
}
...
impl ResourceReloadListener for CloudRendererRebuildCandidateReloadListener {
    fn name(&self) -> &str {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ClientCloudRendererRebuildCandidateReport {
    resource: String,
    pack_id: Option<String>,
    byte_count: Option<usize>,
    width: Option<u32>,
    height: Option<u32>,
    estimated_cell_count: Option<u64>,
    rebuild_needed: bool,
    rebuild_reason: String,
    blockers: Vec<ClientCloudRendererRebuildBlocker>,
}

impl ClientCloudRendererRebuildCandidateReport {
    fn from_render_input(input: CloudRenderInputReport) -> Self {
        let estimated_cell_count = Some(u64::from(input.width()) * u64::from(input.height()));
        Self {
            resource: input.resource,
            pack_id: Some(input.pack_id),
            byte_count: Some(input.byte_count),
            width: Some(input.width),
            height: Some(input.height),
            estimated_cell_count,
            rebuild_needed: input.rebuild_needed,
            rebuild_reason: input.rebuild_reason,
            blockers: Vec::new(),
        }
    }

    fn blocked(resource: impl Into<String>, blocker: ClientCloudRendererRebuildBlocker) -> Self {
        Self {
            resource: resource.into(),
            pack_id: blocker.selected_pack_id.clone(),
    ...
    pub fn estimated_cell_count(&self) -> Option<u64> {
        self.estimated_cell_count
    }
    ...
    pub fn blockers(&self) -> &[ClientCloudRendererRebuildBlocker] {
        &self.blockers
    }

    pub fn candidate_count(&self) -> usize {
        usize::from(self.blockers.is_empty())
    }
    ...
    pub fn summary_fragment(&self) -> String {
        format!(
            "cloud_renderer_rebuild_candidates:resources:1 candidates:{} blocked:{} bytes:{} cells:{} boundary:{}",
            self.candidate_count(),
            self.blocker_count(),
            self.byte_count.unwrap_or(0),
            self.estimated_cell_count.unwrap_or(0),
            CLOUD_RENDERER_REBUILD_BOUNDARY
        )
    }
    ...
    pub fn items(&self) -> Vec<String> {
        ...
        if self.blockers.is_empty() {
            items.push(cloud_renderer_rebuild_candidate_report_item(self));
        } else {
            items.extend(
                self.blockers
                    .iter()
                    .map(cloud_renderer_rebuild_blocker_report_item),
            );
        }
        items
    }
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ClientCloudRendererRebuildBlocker {
    resource: String,
    selected_pack_id: Option<String>,
    byte_count: Option<usize>,
    reason: ClientCloudRendererRebuildBlockerReason,
}

impl ClientCloudRendererRebuildBlocker {
    pub fn resource(&self) -> &str {
        &self.resource
    }
    ...
    pub fn reason(&self) -> &ClientCloudRendererRebuildBlockerReason {
        &self.reason
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ClientCloudRendererRebuildBlockerReason {
    MissingTextureResource,
    ReadTextureResource,
    InvalidPngSignature,
    InvalidPngMetadata { reason: String },
...
impl ClientCloudRendererRebuildBlockerReason {
    fn report_fragment(&self) -> String {
        match self {
            Self::MissingTextureResource => "missing_texture_resource".to_owned(),
            Self::ReadTextureResource => "read_texture_resource".to_owned(),
            Self::InvalidPngSignature => "invalid_png_signature".to_owned(),
            Self::InvalidPngMetadata { reason } => format!("invalid_png_metadata:{reason}"),
        }
    }
...
fn load_cloud_renderer_rebuild_candidate_report(
    stack: &ClientResourceStack,
    resource: &str,
) -> ClientCloudRendererRebuildCandidateReport {
    let Some(location) = stack.find_resource(resource) else {
        return ClientCloudRendererRebuildCandidateReport::blocked(
            resource,
            ClientCloudRendererRebuildBlocker {
                resource: resource.to_owned(),
                selected_pack_id: None,
                byte_count: None,
                reason: ClientCloudRendererRebuildBlockerReason::MissingTextureResource,
            },
        );
    };
        ...
        Err(_) => {
            return ClientCloudRendererRebuildCandidateReport::blocked(
                ...
                ClientCloudRendererRebuildBlocker {
                    ...
                    selected_pack_id: Some(pack_id),
                    byte_count: None,
                    reason: ClientCloudRendererRebuildBlockerReason::ReadTextureResource,
                },
            );
        }
    };
    ...
    match decode_png_dimensions(resource, &location, &bytes) {
        Ok(image) => ClientCloudRendererRebuildCandidateReport::from_render_input(
            CloudRenderInputReport::from_png_dimensions(
                resource,
                location.pack_id,
                byte_count,
                image,
        ...
        Err(ResourceReloadError::InvalidPngSignature { .. }) => {
            ClientCloudRendererRebuildCandidateReport::blocked(
                resource,
                ClientCloudRendererRebuildBlocker {
                    resource: resource.to_owned(),
                    selected_pack_id: Some(pack_id),
                    byte_count: Some(byte_count),
                    reason: ClientCloudRendererRebuildBlockerReason::InvalidPngSignature,
                },
            )
        }
        Err(ResourceReloadError::InvalidPngMetadata { reason, .. }) => {
            ClientCloudRendererRebuildCandidateReport::blocked(
                ...
                ClientCloudRendererRebuildBlocker {
                    ...
                    selected_pack_id: Some(pack_id),
                    byte_count: Some(byte_count),
                    reason: ClientCloudRendererRebuildBlockerReason::InvalidPngMetadata { reason },
                },
            )
        }
        Err(_) => ClientCloudRendererRebuildCandidateReport::blocked(
            resource,
            ClientCloudRendererRebuildBlocker {
                resource: resource.to_owned(),
                selected_pack_id: Some(pack_id),
...
fn cloud_renderer_rebuild_candidate_report_item(
    report: &ClientCloudRendererRebuildCandidateReport,
) -> String {
    format!(
        "cloud_renderer_rebuild_candidate:{}@{}:{} bytes:png:{}x{}:cells:{}:rebuild:{}:{}:boundary:{}",
        report.resource(),
        report.pack_id().unwrap_or("missing"),
        report.byte_count().unwrap_or(0),
        report.width().unwrap_or(0),
        report.height().unwrap_or(0),
        report.estimated_cell_count().unwrap_or(0),
        if report.rebuild_needed() {
            "needed"
        } else {
            "clean"
...
fn cloud_renderer_rebuild_blocker_report_item(
    blocker: &ClientCloudRendererRebuildBlocker,
) -> String {
    format!(
        "cloud_renderer_rebuild_blocker:{}@{}:bytes:{}:reason:{}:boundary:{}",
        blocker.resource(),
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct InitialTextureReloadListener {
    name: String,
}

impl InitialTextureReloadListener {
    pub fn new() -> Self {
        Self {
            name: "initial_textures".to_owned(),
        }
    }
    ...
    pub fn load(
        &self,
        stack: &ClientResourceStack,
    ) -> ResourceReloadResult<InitialTextureReloadReport> {
        load_initial_textures(stack)
    }
}

impl Default for InitialTextureReloadListener {
    fn default() -> Self {
        Self::new()
    }
}

impl ResourceReloadListener for InitialTextureReloadListener {
    fn name(&self) -> &str {
        &self.name
    }
    ...
    fn prepare(
        ...
        for (_, resource) in INITIAL_SIMPLE_TEXTURES {
            stack.require_resource(resource)?;
        }
    ...
    fn reload(
        ...
        Ok(ResourceReloadTaskReport::new(
            self.load(stack)?
                .items()
                .iter()
                .map(initial_texture_report_item),
        ))
    }
}
...
impl InitialTextureReloadReport {
    pub fn items(&self) -> &[InitialTextureReportItem] {
        &self.items
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct InitialTextureReportItem {
    registered_id: String,
    loader_kind: InitialTextureLoaderKind,
...
impl InitialTextureReportItem {
    ...
    pub fn loaded_resource_paths(&self) -> &[String] {
        &self.loaded_resource_paths
    }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum InitialTextureLoaderKind {
    SimpleTexture,
    MojangLogo,
    Cubemap,
}

impl InitialTextureLoaderKind {
    fn report_fragment(self) -> &'static str {
        match self {
            Self::SimpleTexture => "simple_texture",
            Self::MojangLogo => "mojang_logo",
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct InitialTextureMetadata {
    source: InitialTextureMetadataSource,
    blur: bool,
    clamp: bool,
    mipmap_strategy: InitialTextureMipmapStrategy,
    alpha_cutoff_bias: String,
}

impl InitialTextureMetadata {
    fn default_source(source: InitialTextureMetadataSource) -> Self {
        Self {
            source,
            blur: false,
            clamp: false,
            mipmap_strategy: InitialTextureMipmapStrategy::Auto,
            alpha_cutoff_bias: "0.0".to_owned(),
        }
    }

    fn synthetic(blur: bool, clamp: bool) -> Self {
        Self {
            source: InitialTextureMetadataSource::Synthetic,
            blur,
            clamp,
            mipmap_strategy: InitialTextureMipmapStrategy::Mean,
            alpha_cutoff_bias: "0.0".to_owned(),
        }
    }

    pub fn source(&self) -> &InitialTextureMetadataSource {
        &self.source
    }
    ...
    pub fn mipmap_strategy(&self) -> InitialTextureMipmapStrategy {
        self.mipmap_strategy
    }

    pub fn alpha_cutoff_bias(&self) -> &str {
        &self.alpha_cutoff_bias
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum InitialTextureMetadataSource {
    Synthetic,
    Default,
    Resource { resource: String, pack_id: String },
}

impl InitialTextureMetadataSource {
    fn report_fragment(&self) -> String {
        match self {
            Self::Synthetic => "synthetic".to_owned(),
            Self::Default => "default".to_owned(),
            Self::Resource { resource, pack_id } => format!("resource:{resource}@{pack_id}"),
        }
    }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum InitialTextureMipmapStrategy {
    Auto,
    Mean,
    Cutout,
    StrictCutout,
...
impl InitialTextureMipmapStrategy {
    fn from_str(value: &str) -> Option<Self> {
        match value {
            "auto" => Some(Self::Auto),
            "mean" => Some(Self::Mean),
            "cutout" => Some(Self::Cutout),
            "strict_cutout" => Some(Self::StrictCutout),
            "dark_cutout" => Some(Self::DarkCutout),
            _ => None,
        }
    }
    ...
    fn report_fragment(self) -> &'static str {
        match self {
            ...
            Self::Mean => "mean",
            Self::Cutout => "cutout",
            Self::StrictCutout => "strict_cutout",
            Self::DarkCutout => "dark_cutout",
        }
    }
}

fn load_initial_textures(
    stack: &ClientResourceStack,
) -> ResourceReloadResult<InitialTextureReloadReport> {
    let mut items = Vec::with_capacity(INITIAL_SIMPLE_TEXTURES.len() + 2);

    for (registered_id, resource) in INITIAL_SIMPLE_TEXTURES {
        items.push(load_initial_simple_texture(stack, registered_id, resource)?);
    }
    items.push(load_initial_mojang_logo(stack)?);
    items.push(load_initial_cubemap(stack)?);

    Ok(InitialTextureReloadReport { items })
}

fn load_initial_simple_texture(
    stack: &ClientResourceStack,
    registered_id: &str,
    resource: &str,
) -> ResourceReloadResult<InitialTextureReportItem> {
    let (pack_index, location) = require_resource_with_pack_index(stack, resource)?;
    let bytes = read_resource_bytes(resource, &location)?;
    let image = decode_png_dimensions(resource, &location, &bytes)?;
    let metadata = load_initial_texture_metadata(stack, resource, pack_index)?;
...
fn load_loading_overlay_logo_texture(
    ...
    Ok(InitialTextureReportItem {
        ...
        height: image.height,
        depth: 1,
        metadata: InitialTextureMetadata::synthetic(true, true),
    })
}
...
fn load_cubemap_texture(
    ...
    for resource in &resources {
        ...
        pack_ids.push(location.pack_id);
        byte_counts.push(bytes.len());
    }
    ...
    Ok(InitialTextureReportItem {
        ...
        depth: INITIAL_CUBEMAP_SUFFIXES.len() as u32,
        metadata: InitialTextureMetadata::synthetic(true, false),
    })
}
...
fn initial_texture_prepare_items() -> Vec<String> {
    let mut items = INITIAL_SIMPLE_TEXTURES
        .iter()
        .map(|(_, resource)| (*resource).to_owned())
        .collect::<Vec<_>>();
    items.push(INITIAL_MOJANG_LOGO_RESOURCE.to_owned());
    items.extend(initial_cubemap_resources());
    items
}

fn initial_texture_report_item(item: &InitialTextureReportItem) -> String {
    format!(
        "{} kind:{} resources:{} packs:{} bytes:{} size:{}x{}x{} metadata:{} blur:{} clamp:{} mipmap_strategy:{} alpha_cutoff_bias:{}",
        item.registered_id,
        item.loader_kind.report_fragment(),
        item.loaded_resource_paths.join(","),
        item.pack_ids.join(","),
        item.byte_counts
            .iter()
            .map(usize::to_string)
        ...
        item.height,
        item.depth,
        item.metadata.source.report_fragment(),
        item.metadata.blur,
        item.metadata.clamp,
...
fn load_initial_texture_metadata(
    ...
    texture_pack_index: usize,
) -> ResourceReloadResult<InitialTextureMetadata> {
    let metadata_resource = format!("{texture_resource}.mcmeta");
    for pack in stack
        .packs
        .iter()
        .enumerate()
        ...
        .filter_map(|(index, pack)| {
            (index >= texture_pack_index)
                .then(|| pack.resource_location(Path::new(&metadata_resource)))
                .flatten()
        })
    {
        let metadata = read_initial_texture_metadata(&metadata_resource, &pack)?;
        return Ok(InitialTextureMetadata {
            source: InitialTextureMetadataSource::Resource {
    ...
    Ok(InitialTextureMetadata::default_source(
        InitialTextureMetadataSource::Default,
    ))
}

fn read_initial_texture_metadata(
    ...
    let contents =
        location
            .read_to_string()
            .map_err(|source| ResourceReloadError::ReadResource {
                resource: metadata_resource.to_owned(),
                path: location.path.clone(),
                source,
            })?;
    let value = serde_json::from_str::<serde_json::Value>(&contents).map_err(|source| {
    ...
    parse_initial_texture_metadata_value(metadata_resource, &location.pack_id, &value)
}

fn parse_initial_texture_metadata_value(
    resource: &str,
    pack_id: &str,
    value: &serde_json::Value,
) -> ResourceReloadResult<InitialTextureMetadata> {
    let Some(root) = value.as_object() else {
        return Err(invalid_initial_texture_metadata(
            resource,
            pack_id,
            "top-level metadata is not an object",
        ));
    };
    let Some(texture) = root.get("texture") else {
        return Ok(InitialTextureMetadata::default_source(
            InitialTextureMetadataSource::Default,
        ));
    };
    let Some(texture) = texture.as_object() else {
        return Err(invalid_initial_texture_metadata(
            resource,
            pack_id,
            "texture section is not an object",
        ));
    };

    let mut metadata =
        InitialTextureMetadata::default_source(InitialTextureMetadataSource::Default);
    if let Some(value) = texture.get("blur") {
        metadata.blur = value.as_bool().ok_or_else(|| {
            invalid_initial_texture_metadata(resource, pack_id, "texture.blur is not a bool")
        })?;
    }
    if let Some(value) = texture.get("clamp") {
        metadata.clamp = value.as_bool().ok_or_else(|| {
            invalid_initial_texture_metadata(resource, pack_id, "texture.clamp is not a bool")
        })?;
    }
    if let Some(value) = texture.get("mipmap_strategy") {
        let value = value.as_str().ok_or_else(|| {
            invalid_initial_texture_metadata(
                resource,
                pack_id,
                "texture.mipmap_strategy is not a string",
            )
        })?;
        metadata.mipmap_strategy =
            InitialTextureMipmapStrategy::from_str(value).ok_or_else(|| {
                invalid_initial_texture_metadata(
                    resource,
                    pack_id,
                    format!("unknown texture.mipmap_strategy `{value}`"),
                )
            })?;
    }
    if let Some(value) = texture.get("alpha_cutoff_bias") {
        let number = value.as_f64().ok_or_else(|| {
            invalid_initial_texture_metadata(
                resource,
                pack_id,
                "texture.alpha_cutoff_bias is not a number",
            )
        })?;
        metadata.alpha_cutoff_bias = number.to_string();
        if !metadata.alpha_cutoff_bias.contains('.') {
            metadata.alpha_cutoff_bias.push_str(".0");
        }
    }
...
fn invalid_initial_texture_metadata(
    resource: &str,
    pack_id: &str,
    reason: impl Into<String>,
) -> ResourceReloadError {
...
fn require_resource_with_pack_index(
    stack: &ClientResourceStack,
    resource: &str,
) -> ResourceReloadResult<(usize, ResourceLocation)> {
    stack
        .packs
        .iter()
        .enumerate()
        .rev()
        .find_map(|(index, pack)| {
            pack.resource_location(Path::new(resource))
                .map(|location| (index, location))
        })
        .ok_or_else(|| ResourceReloadError::MissingResource(resource.to_owned()))
}

fn require_resource_from_pack(
    stack: &ClientResourceStack,
    resource: &str,
    pack_id: &str,
) -> ResourceReloadResult<ResourceLocation> {
    stack
        ...
        .and_then(|pack| pack.resource_location(Path::new(resource)))
        .ok_or_else(|| ResourceReloadError::MissingResource(format!("{resource}@{pack_id}")))
}
...
fn decode_png_dimensions(
    ...
    location: &ResourceLocation,
    bytes: &[u8],
) -> ResourceReloadResult<DecodedPngDimensions> {
...
pub enum ResourceReloadError {
    ...
    #[error("invalid initial texture `{registered_id}` resource `{resource}`: {reason}")]
    InvalidInitialTexture {
        registered_id: String,
        resource: String,
        reason: String,
    },
    #[error("invalid initial texture metadata `{resource}` from pack `{pack_id}`: {reason}")]
    InvalidInitialTextureMetadata {
        resource: String,
        pack_id: String,
        reason: String,
    },
...
mod tests {
    ...
    #[test]
    fn sound_preload_reports_available_file_candidate_with_bytes() {
        let temp = TempPack::new();
        temp.write(
            SOUND_EVENTS_RESOURCE,
            r#"{"music.menu":{"sounds":[{"name":"music.menu","preload":true},{"name":"music.extra"}]}}"#,
        ...
        let sounds = SoundEventsReloadListener::default()
            ...
            .expect("preload sound events should load");
        let candidates = sounds.report().preload_buffer_candidates();

        assert_eq!(candidates.len(), 1);
        assert_eq!(candidates[0].event_id(), "minecraft:music.menu");
        assert_eq!(candidates[0].sound_name(), "music.menu");
        assert_eq!(
            candidates[0].file_resource(),
            "assets/minecraft/sounds/music.menu.ogg"
        );
        assert_eq!(candidates[0].pack_id(), Some("test"));
        assert_eq!(candidates[0].byte_count(), Some(4));
        assert!(candidates[0].missing_blockers().is_empty());
        assert!(
            sounds
                .report()
                .summary_fragment()
    ...
    fn sound_preload_excludes_non_preload_file_candidates() {
        ...
        temp.write(
            SOUND_EVENTS_RESOURCE,
            r#"{"music.menu":{"sounds":[{"name":"music.menu"},{"name":"music.extra","preload":false}]}}"#,
        );
        temp.write_bytes("assets/minecraft/sounds/music.menu.ogg", b"menu");
        temp.write_bytes("assets/minecraft/sounds/music.extra.ogg", b"extra");
        ...
        let sounds = SoundEventsReloadListener::default()
            ...
            .expect("non-preload sound events should load");

        assert_eq!(sounds.report().file_sound_count(), 2);
        assert!(sounds.report().preload_buffer_candidates().is_empty());
    }

    #[test]
    fn sound_preload_excludes_event_reference_candidates() {
        let temp = TempPack::new();
        temp.write(
            SOUND_EVENTS_RESOURCE,
            r#"{"ui.parent":{"sounds":[{"name":"ui.child","type":"event","preload":true}]},"ui.child":{"sounds":["ui.child_file"]}}"#,
        );
        temp.write_bytes("assets/minecraft/sounds/ui.child_file.ogg", b"child");
        ...
        let sounds = SoundEventsReloadListener::default()
            ...
            .expect("event reference sound events should load");
    ...
    #[test]
    fn sound_preload_reports_missing_file_blocker() {
        let temp = TempPack::new();
        temp.write(
            SOUND_EVENTS_RESOURCE,
        ...
        let sounds = SoundEventsReloadListener::default()
            ...
            .expect("missing preload files should be reported, not panic");
        let candidates = sounds.report().preload_buffer_candidates();

        assert_eq!(sounds.report().missing_file_sound_count(), 1);
        assert_eq!(candidates.len(), 1);
        assert_eq!(candidates[0].event_id(), "minecraft:music.menu");
        assert_eq!(candidates[0].sound_name(), "missing.music");
        assert_eq!(
            candidates[0].file_resource(),
            "assets/minecraft/sounds/missing.music.ogg"
        );
        assert_eq!(candidates[0].pack_id(), None);
        assert_eq!(candidates[0].byte_count(), None);
        assert_eq!(
            candidates[0].missing_blockers(),
            &[
    ...
    #[test]
    fn sound_preload_uses_top_priority_resource_for_pack_and_byte_count() {
        let base = TempPack::new();
        let override_pack = TempPack::new();
        base.write(
            SOUND_EVENTS_RESOURCE,
            r#"{"test.priority":{"sounds":[{"name":"priority.sound","preload":true}]}}"#,
        );
        ...
        let sounds = SoundEventsReloadListener::default()
            ...
            .expect("priority preload sound events should load");
        let candidates = sounds.report().preload_buffer_candidates();

        assert_eq!(candidates.len(), 1);
        assert_eq!(candidates[0].pack_id(), Some("override"));
        assert_eq!(candidates[0].byte_count(), Some("override".len()));
    }
    ...
    #[test]
    fn waypoint_asset_report_groups_available_and_missing_sprite_resources_by_style_id() {
        let temp = TempPack::new();
        temp.write(
            "assets/minecraft/waypoint_style/default.json",
            r#"{"sprites":["minecraft:default_0","custom:pin","minecraft:missing"]}"#,
        );
        temp.write_bytes(
        ...
        let stack = ClientResourceStack::new(vec![ClientResourcePack::new("test", temp.path())]);
        let assets = WaypointStyleAssetReloadListener::new(["default"])
            .load(&stack)
            .expect("missing waypoint sprite resources should report without failing reload");
        let style = &assets.styles()[0];

        assert_eq!(style.style_id(), "default");
        assert_eq!(style.sprite_count(), 3);
        assert_eq!(style.available_resource_count(), 2);
        assert_eq!(style.missing_resource_count(), 1);
        assert_eq!(
            style
                .available_resources()
                .iter()
                .map(ClientWaypointStyleAssetAvailability::resource)
                .collect::<Vec<_>>(),
            [
                "assets/minecraft/textures/gui/sprites/hud/locator_bar_dot/default_0.png",
                "assets/custom/textures/gui/sprites/hud/locator_bar_dot/pin.png",
        ...
        assert_eq!(
            style.blockers()[0].resource(),
            "assets/minecraft/textures/gui/sprites/hud/locator_bar_dot/missing.png"
        );
        assert_eq!(
            style.blockers()[0].reason(),
            ClientWaypointStyleAssetBlockerReason::MissingTextureResource
        );

        let report = ResourceReloadManager::new(stack)
            .with_listener(WaypointStyleAssetReloadListener::new(["default"]))
            .run()
            .expect("missing waypoint sprite resources should be blockers, not reload errors");
        assert_eq!(
            report.listener_reports()[0].reload.items(),
            ["assets/minecraft/waypoint_style/default.json@test id:default sprites:3:minecraft:default_0,custom:pin,minecraft:missing locations:minecraft:hud/locator_bar_dot/default_0,custom:hud/locator_bar_dot/pin,minecraft:hud/locator_bar_dot/missing available:2 missing:1 resources:minecraft:hud/locator_bar_dot/default_0=assets/minecraft/textures/gui/sprites/hud/locator_bar_dot/default_0.png@test,custom:hud/locator_bar_dot/pin=assets/custom/textures/gui/sprites/hud/locator_bar_dot/pin.png@test blockers:missing:minecraft:hud/locator_bar_dot/missing=assets/minecraft/textures/gui/sprites/hud/locator_bar_dot/missing.png".to_owned()]
        );
    }

    #[test]
    fn waypoint_asset_report_uses_stack_priority_for_sprite_resources() {
        let base = TempPack::new();
        let override_pack = TempPack::new();
        base.write(
            "assets/minecraft/waypoint_style/default.json",
        ...
        let assets = WaypointStyleAssetReloadListener::new(["default"])
            .load(&stack)
            .expect("waypoint sprite asset resources should load");
        let style = &assets.styles()[0];

        assert_eq!(style.available_resource_count(), 1);
        assert_eq!(
            style.available_resources()[0].resource(),
            "assets/minecraft/textures/gui/sprites/hud/locator_bar_dot/default_0.png"
        );
        assert_eq!(style.available_resources()[0].pack_id(), "override");
        assert!(style.blockers().is_empty());
    }
    ...
    #[test]
    fn equipment_render_candidates_decode_pngs_and_report_boundary() {
        let temp = TempPack::new();
        let humanoid_png = encode_test_rgba_png(2, 3, &[0, 0, 0, 255].repeat(6));
        let wings_png = encode_test_rgba_png(1, 1, &[255, 255, 255, 255]);
        temp.write(
        ...
        let report = ResourceReloadManager::new(stack)
            .with_listener(EquipmentRenderAssetCandidateReloadListener::default())
            .run()
            .expect("equipment render candidates should decode without renderer integration");

        let listener = &report.listener_reports()[0];
        assert_eq!(listener.name, "equipment_render_asset_candidates");
        assert_eq!(
            listener.reload.items()[0],
            format!(
                "equipment_render_asset_candidates:equipment:1 layers:2 textures:2 available:2 decoded:2 blocked:0 bytes:{} boundary:texture_decode_complete_equipment_renderer_pending",
                humanoid_png.len() + wings_png.len()
            )
        );
        assert!(
            listener.reload.items()[1].contains(
                "layer_candidates:humanoid:minecraft:custom=minecraft:textures/entity/equipment/humanoid/custom.png resource:assets/minecraft/textures/entity/equipment/humanoid/custom.png dyeable:color_when_undyed:-6265536 player_texture:false"
            ) && listener.reload.items()[1].contains(
                "wings:custom:wing=custom:textures/entity/equipment/wings/wing.png resource:assets/custom/textures/entity/equipment/wings/wing.png dyeable:none player_texture:true"
            )
        );
        assert!(
            listener.reload.items()[1].contains(
                "resources:humanoid:minecraft:custom=assets/minecraft/textures/entity/equipment/humanoid/custom.png@test:"
            ) && listener.reload.items()[1].contains(" bytes:png:2x3 dyeable:color_when_undyed:-6265536 player_texture:false")
                && listener.reload.items()[1].contains(
                    "wings:custom:wing=assets/custom/textures/entity/equipment/wings/wing.png@test:"
                )
                && listener.reload.items()[1].contains(" bytes:png:1x1 dyeable:none player_texture:true")
                && listener.reload.items()[1].contains("blocker_details:none")
                && listener.reload.items()[1]
                    .ends_with("boundary:texture_decode_complete_equipment_renderer_pending")
        );
    }

    #[test]
    fn equipment_render_candidates_report_missing_and_invalid_texture_blockers() {
        let temp = TempPack::new();
        let valid_png = encode_test_rgba_png(1, 1, &[0, 0, 0, 255]);
        temp.write(
            "assets/minecraft/equipment/custom.json",
            r#"{"layers":{"humanoid":[{"texture":"minecraft:valid"},{"texture":"minecraft:missing"},{"texture":"minecraft:corrupt","dyeable":{}}]}}"#,
        );
        temp.write_bytes(
        ...
        let report = ResourceReloadManager::new(stack)
            .with_listener(EquipmentRenderAssetCandidateReloadListener::default())
            .run()
            .expect("missing and invalid equipment textures should be blockers");

        let listener = &report.listener_reports()[0];
        assert_eq!(
            listener.reload.items()[0],
            format!(
                "equipment_render_asset_candidates:equipment:1 layers:1 textures:3 available:2 decoded:1 blocked:2 bytes:{} boundary:texture_decode_complete_equipment_renderer_pending",
                valid_png.len()
            )
        );
        assert!(listener.reload.items()[1].contains(
            "resources:humanoid:minecraft:valid=assets/minecraft/textures/entity/equipment/humanoid/valid.png@test:"
        ));
        assert!(
            listener.reload.items()[1].contains(
                "missing:humanoid:minecraft:missing=assets/minecraft/textures/entity/equipment/humanoid/missing.png@none bytes:unknown dyeable:none player_texture:false"
            ) && listener.reload.items()[1].contains(
                "invalid_png_signature:humanoid:minecraft:corrupt=assets/minecraft/textures/entity/equipment/humanoid/corrupt.png@test bytes:9 dyeable:color_when_undyed:none player_texture:false"
            )
        );
    }

    #[test]
    fn equipment_render_candidates_use_stack_priority_for_texture_decode() {
        let base = TempPack::new();
        let override_pack = TempPack::new();
        let base_png = encode_test_rgba_png(1, 1, &[0, 0, 0, 255]);
        let override_png = encode_test_rgba_png(2, 2, &[255, 0, 0, 255].repeat(4));
        base.write(
            "assets/minecraft/equipment/custom.json",
            r#"{"layers":{"horse_body":[{"texture":"minecraft:custom"}]}}"#,
        );
        base.write_bytes(
            "assets/minecraft/textures/entity/equipment/horse_body/custom.png",
            &base_png,
        );
        override_pack.write_bytes(
            "assets/minecraft/textures/entity/equipment/horse_body/custom.png",
        ...
        let candidates = EquipmentRenderAssetCandidateReloadListener::default()
            .load(&stack)
            .expect("equipment render candidate should load");
        let decoded = &candidates.equipment()[0].layers()[0].decoded_texture_resources()[0];
    ...
    #[test]
    fn committed_vanilla_waypoint_asset_report_has_nonzero_sprite_surface() {
        let assets = WaypointStyleAssetReloadListener::default()
            .load(&ClientResourceStack::vanilla())
            .expect("committed vanilla waypoint sprite assets should load");
        let total_sprites = assets
            .styles()
            .iter()
            .map(ClientWaypointStyleAssetCandidate::sprite_count)
            .sum::<usize>();
        let total_available = assets
            .styles()
            .iter()
            .map(ClientWaypointStyleAssetCandidate::available_resource_count)
            .sum::<usize>();

        assert_eq!(assets.styles().len(), 2);
        assert!(total_sprites > 0);
        assert_eq!(total_sprites, total_available);
        assert!(assets.styles().iter().any(|style| {
            style.style_id() == "default"
                && style.available_resources().iter().any(|availability| {
                    availability.sprite_location() == "minecraft:hud/locator_bar_dot/default_0"
                        && availability.resource()
                            == "assets/minecraft/textures/gui/sprites/hud/locator_bar_dot/default_0.png"
                        && availability.pack_id() == VANILLA_PACK_ID
                })
    ...
    #[test]
    fn committed_vanilla_equipment_render_candidates_decode_representative_textures() {
        let candidates = EquipmentRenderAssetCandidateReloadListener::default()
            .load(&ClientResourceStack::vanilla())
            .expect("committed vanilla equipment render candidates should load");

        assert!(candidates.equipment().len() >= 40);
        assert!(candidates.layer_count() > candidates.equipment_count());
        assert!(candidates.texture_count() > 0);
        assert_eq!(
            candidates.texture_count(),
            candidates.available_resource_count()
        );
        assert_eq!(
            candidates.available_resource_count(),
        ...
        assert!(
            candidates
                .summary_fragment()
                .contains("boundary:texture_decode_complete_equipment_renderer_pending")
        );
        assert!(candidates.equipment().iter().any(|equipment| {
            equipment.equipment_id() == "diamond"
                && equipment.layers().iter().any(|layer| {
                    layer.layer_type() == "humanoid"
                    && layer.decoded_texture_resources().iter().any(|resource| {
                        resource.texture_id() == "minecraft:diamond"
                            && resource.resource()
        ...
        assert!(candidates.equipment().iter().any(|equipment| {
            equipment.equipment_id() == "leather"
                && equipment.layers().iter().any(|layer| {
                    layer.layer_type() == "humanoid"
                        && layer.decoded_texture_resources().iter().any(|resource| {
                            resource.texture_id() == "minecraft:leather"
                                && resource
                                    .dyeable()
                                    .and_then(ClientEquipmentDyeable::color_when_undyed)
                                    == Some(-6265536)
                        })
                })
        }));
    ...
    #[test]
    fn initial_texture_listener_uses_highest_texture_and_same_or_higher_metadata() {
        let base = TempPack::new();
        ...
        let item = load_initial_simple_texture(&stack, INITIAL_SIMPLE_TEXTURES[0].0, resource)
            .expect("initial simple texture should load");

        assert_eq!(item.pack_ids(), ["texture".to_owned()]);
        assert_eq!(item.byte_counts(), [OVERRIDE_MINIMAL_PNG.len()]);
        assert_eq!(
            item.metadata().source(),
            &InitialTextureMetadataSource::Resource {
                resource: format!("{resource}.mcmeta"),
                pack_id: "metadata".to_owned()
            }
        );
        assert!(item.metadata().blur());
        assert!(item.metadata().clamp());
        assert_eq!(
            item.metadata().mipmap_strategy(),
            InitialTextureMipmapStrategy::Cutout
        );
        assert_eq!(item.metadata().alpha_cutoff_bias(), "0.25");
    }

    #[test]
    fn initial_texture_listener_ignores_lower_priority_mcmeta_when_higher_texture_wins() {
        let base = TempPack::new();
        let override_pack = TempPack::new();
        let resource = INITIAL_SIMPLE_TEXTURES[1].1;
        base.write_bytes(resource, MINIMAL_PNG);
        base.write(
            &format!("{resource}.mcmeta"),
            r#"{"texture":{"blur":true,"clamp":true,"mipmap_strategy":"mean","alpha_cutoff_bias":1.0}}"#,
        );
        override_pack.write_bytes(resource, OVERRIDE_MINIMAL_PNG);

        let stack = ClientResourceStack::new(vec![
            ClientResourcePack::new("base", base.path()),
            ClientResourcePack::new("override", override_pack.path()),
        ]);
        let item = load_initial_simple_texture(&stack, INITIAL_SIMPLE_TEXTURES[1].0, resource)
            .expect("initial simple texture should load");

        assert_eq!(item.pack_ids(), ["override".to_owned()]);
        assert_eq!(
            item.metadata().source(),
            &InitialTextureMetadataSource::Default
        );
        ...
        assert_eq!(
            item.metadata().mipmap_strategy(),
            InitialTextureMipmapStrategy::Auto
        );
        assert_eq!(item.metadata().alpha_cutoff_bias(), "0.0");
    }

    #[test]
    fn initial_texture_listener_rejects_invalid_texture_mcmeta_section() {
        let temp = TempPack::new();
        let resource = INITIAL_SIMPLE_TEXTURES[2].1;
        let png = encode_test_rgba_png(1, 1, &[0, 0, 0, 255]);
        temp.write_bytes(resource, &png);
        temp.write(
            &format!("{resource}.mcmeta"),
            r#"{"texture":{"blur":"yes"}}"#,
        );
        ...
        let stack = ClientResourceStack::new(vec![ClientResourcePack::new("test", temp.path())]);
        let error = load_initial_simple_texture(&stack, INITIAL_SIMPLE_TEXTURES[2].0, resource)
            .expect_err("invalid texture mcmeta should fail");

        assert!(
            matches!(error, ResourceReloadError::InvalidInitialTextureMetadata { resource: error_resource, pack_id, .. } if error_resource == format!("{resource}.mcmeta") && pack_id == "test")
        );
    }

    #[test]
    fn initial_texture_listener_rejects_corrupt_png() {
        let temp = TempPack::new();
        let resource = INITIAL_SIMPLE_TEXTURES[0].1;
        temp.write_bytes(resource, b"not a png");

        let stack = ClientResourceStack::new(vec![ClientResourcePack::new("test", temp.path())]);
        let error = load_initial_simple_texture(&stack, INITIAL_SIMPLE_TEXTURES[0].0, resource)
    ...
    #[test]
    fn initial_texture_cubemap_reports_suffix_order_and_rejects_dimension_mismatch() {
        let temp = TempPack::new();
        ...
        let stack = ClientResourceStack::new(vec![ClientResourcePack::new("test", temp.path())]);
        let item = load_initial_cubemap(&stack).expect("cubemap should load");
        assert_eq!(item.loaded_resource_paths(), resources);
        assert_eq!(item.width(), 2);
        assert_eq!(item.height(), 2);
        assert_eq!(item.depth(), 6);

        let mismatch = TempPack::new();
        let png_1x1 = encode_test_rgba_png(1, 1, &[255, 0, 0, 255]);
        for (index, resource) in resources.iter().enumerate() {
            mismatch.write_bytes(resource, if index == 3 { &png_1x1 } else { &png_2x2 });
        }
        let stack =
            ClientResourceStack::new(vec![ClientResourcePack::new("mismatch", mismatch.path())]);
        let error = load_initial_cubemap(&stack).expect_err("mismatched cubemap should fail");

        assert!(
            matches!(error, ResourceReloadError::InvalidInitialTexture { registered_id, resource, .. } if registered_id == INITIAL_CUBEMAP_ID && resource.ends_with("_4.png"))
        );
    }

    #[test]
    fn committed_vanilla_initial_texture_listener_loads_startup_texture_set() {
        let report = ResourceReloadManager::new(ClientResourceStack::vanilla())
            .with_listener(InitialTextureReloadListener::default())
            .run()
            .expect("committed vanilla initial texture set should load");

        let listener = &report.listener_reports()[0];
        assert_eq!(listener.name, "initial_textures");
        assert_eq!(
            listener.preparation.items(),
            initial_texture_prepare_items()
        );
        ...
        assert!(listener.reload.items().iter().any(|item| {
            item.starts_with("minecraft:textures/gui/title/mojangstudios.png kind:mojang_logo")
        }));
        assert!(listener.reload.items().iter().any(|item| {
    ...
    #[test]
    fn cloud_renderer_rebuild_candidate_reports_texture_cells_and_boundary() {
        let temp = TempPack::new();
        let cloud_png = encode_test_rgba_png(
            3,
            2,
        ...
        let stack = ClientResourceStack::new(vec![ClientResourcePack::new("test", temp.path())]);
        let listener = CloudRendererRebuildCandidateReloadListener::default();
        let candidate = listener.load(&stack);

        assert_eq!(candidate.resource(), CLOUDS_TEXTURE_RESOURCE);
        assert_eq!(candidate.pack_id(), Some("test"));
        assert_eq!(candidate.byte_count(), Some(cloud_png.len()));
        assert_eq!(candidate.width(), Some(3));
        assert_eq!(candidate.height(), Some(2));
        assert_eq!(candidate.estimated_cell_count(), Some(6));
        assert_eq!(candidate.candidate_count(), 1);
        assert_eq!(candidate.blocker_count(), 0);
        assert!(candidate.rebuild_needed());
        assert_eq!(candidate.rebuild_reason(), "resource-reload");
        assert_eq!(
            candidate.summary_fragment(),
            format!(
                "cloud_renderer_rebuild_candidates:resources:1 candidates:1 blocked:0 bytes:{} cells:6 boundary:{CLOUD_RENDERER_REBUILD_BOUNDARY}",
                cloud_png.len()
            )
        );
        ...
        let report = ResourceReloadManager::new(stack)
            .with_listener(listener)
            .run()
            .expect("cloud renderer rebuild candidate reload should succeed");

        assert_eq!(
            report.listener_reports()[0].name,
            "cloud_renderer_rebuild_candidates"
        );
        assert_eq!(
            report.listener_reports()[0].reload.items(),
            [
                candidate.summary_fragment(),
                format!(
                    "cloud_renderer_rebuild_candidate:{CLOUDS_TEXTURE_RESOURCE}@test:{} bytes:png:3x2:cells:6:rebuild:needed:resource-reload:boundary:{CLOUD_RENDERER_REBUILD_BOUNDARY}",
                    cloud_png.len()
                )
            ]
        );
    ...
    #[test]
    fn cloud_renderer_rebuild_candidate_uses_highest_priority_texture_pack() {
        let base = TempPack::new();
        let override_pack = TempPack::new();
        let base_png = encode_test_rgba_png(1, 1, &[255, 255, 255, 255]);
        let override_png = encode_test_rgba_png(
        ...
        let candidate = CloudRendererRebuildCandidateReloadListener::default().load(&stack);

        assert_eq!(candidate.pack_id(), Some("override"));
        assert_eq!(candidate.byte_count(), Some(override_png.len()));
        assert_eq!(candidate.width(), Some(2));
        assert_eq!(candidate.height(), Some(2));
        assert_eq!(candidate.estimated_cell_count(), Some(4));
        assert!(candidate.blockers().is_empty());
    }

    #[test]
    fn cloud_renderer_rebuild_candidate_reports_missing_texture_blocker() {
        let stack = ClientResourceStack::new(Vec::new());
        let candidate = CloudRendererRebuildCandidateReloadListener::default().load(&stack);

        assert_eq!(candidate.candidate_count(), 0);
        assert_eq!(candidate.blocker_count(), 1);
        assert_eq!(candidate.pack_id(), None);
        assert_eq!(candidate.byte_count(), None);
        assert_eq!(candidate.estimated_cell_count(), None);
        assert_eq!(candidate.rebuild_reason(), "blocked");
        assert_eq!(
            candidate.blockers()[0].reason(),
            &ClientCloudRendererRebuildBlockerReason::MissingTextureResource
        );
        assert_eq!(
            candidate.items(),
            [
                format!(
                    "cloud_renderer_rebuild_candidates:resources:1 candidates:0 blocked:1 bytes:0 cells:0 boundary:{CLOUD_RENDERER_REBUILD_BOUNDARY}"
                ),
                format!(
                    "cloud_renderer_rebuild_blocker:{CLOUDS_TEXTURE_RESOURCE}@missing:bytes:unavailable:reason:missing_texture_resource:boundary:{CLOUD_RENDERER_REBUILD_BOUNDARY}"
                )
            ]
        );
    }
    ...
    #[test]
    fn cloud_renderer_rebuild_candidate_reports_invalid_png_blocker() {
        let temp = TempPack::new();
        ...
        assert_eq!(candidate.blocker_count(), 1);
        assert_eq!(candidate.pack_id(), Some("test"));
        assert_eq!(candidate.byte_count(), Some(9));
        assert_eq!(
            candidate.blockers()[0].reason(),
            &ClientCloudRendererRebuildBlockerReason::InvalidPngSignature
        );
        assert_eq!(
            candidate.items()[1],
            format!(
                "cloud_renderer_rebuild_blocker:{CLOUDS_TEXTURE_RESOURCE}@test:bytes:9:reason:invalid_png_signature:boundary:{CLOUD_RENDERER_REBUILD_BOUNDARY}"
            )
        );
    }

    #[test]
    fn committed_vanilla_cloud_renderer_rebuild_candidate_reports_boundary_surface() {
        let report = ResourceReloadManager::new(ClientResourceStack::vanilla())
            .with_listener(CloudRendererRebuildCandidateReloadListener::default())
            .run()
            .expect("committed vanilla cloud renderer rebuild candidate should load");

        let listener = &report.listener_reports()[0];
        assert_eq!(listener.name, "cloud_renderer_rebuild_candidates");
        assert_eq!(
            listener.preparation.items(),
            [CLOUDS_TEXTURE_RESOURCE.to_owned()]
        );
        ...
        assert!(listener.reload.items()[1].starts_with(&format!(
            "cloud_renderer_rebuild_candidate:{CLOUDS_TEXTURE_RESOURCE}@{VANILLA_PACK_ID}:"
        )));
        assert!(listener.reload.items()[1].contains(" bytes:png:"));
        assert!(listener.reload.items()[1].contains(":cells:"));
        assert!(listener.reload.items()[1].contains(":rebuild:needed:resource-reload:"));
        assert!(
            listener.reload.items()[1]
                .contains(&format!("boundary:{CLOUD_RENDERER_REBUILD_BOUNDARY}"))
        );
