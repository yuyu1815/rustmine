...
fn login_acknowledged_serverbound_framed_dispatch_body() {
    let manifest: TestManifest = read_json("oracle/test-manifests/775/login_acknowledged_serverbound_framed_dispatch.test-manifest.json");
    assert_eq!(manifest.case_id, "login_acknowledged_serverbound_framed_dispatch");
    assert_eq!(
        manifest.contract_path,
        "oracle/contracts/775/login_acknowledged_serverbound_framed_dispatch.contract.json"
    );
    assert_eq!(manifest.answer_path, "oracle/answers/775/login_acknowledged_serverbound_framed_dispatch.answer.jsonl");
    ...
    assert_runner_scope("oracle/test-manifests/775/login_acknowledged_serverbound_framed_dispatch.test-manifest.json", &manifest);
